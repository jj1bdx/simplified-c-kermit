...
static CHAR p_tbl[] = { // Even parity table for dopar().
    ...
    (CHAR)'\003', (CHAR)'\204', (CHAR)'\005', (CHAR)'\006', (CHAR)'\207',
    (CHAR)'\210', (CHAR)'\011', (CHAR)'\012', (CHAR)'\213', (CHAR)'\014',
    (CHAR)'\215', (CHAR)'\216', (CHAR)'\017', (CHAR)'\220', (CHAR)'\021',
    (CHAR)'\022', (CHAR)'\223', (CHAR)'\024', (CHAR)'\225', (CHAR)'\226',
    (CHAR)'\027', (CHAR)'\030', (CHAR)'\231', (CHAR)'\232', (CHAR)'\033',
    (CHAR)'\234', (CHAR)'\035', (CHAR)'\036', (CHAR)'\237', (CHAR)'\240',
    (CHAR)'\041', (CHAR)'\042', (CHAR)'\243', (CHAR)'\044', (CHAR)'\245',
    (CHAR)'\246', (CHAR)'\047', (CHAR)'\050', (CHAR)'\251', (CHAR)'\252',
    (CHAR)'\053', (CHAR)'\254', (CHAR)'\055', (CHAR)'\056', (CHAR)'\257',
    (CHAR)'\060', (CHAR)'\261', (CHAR)'\262', (CHAR)'\063', (CHAR)'\264',
    (CHAR)'\065', (CHAR)'\066', (CHAR)'\267', (CHAR)'\270', (CHAR)'\071',
    (CHAR)'\072', (CHAR)'\273', (CHAR)'\074', (CHAR)'\275', (CHAR)'\276',
    (CHAR)'\077', (CHAR)'\300', (CHAR)'\101', (CHAR)'\102', (CHAR)'\303',
    (CHAR)'\104', (CHAR)'\305', (CHAR)'\306', (CHAR)'\107', (CHAR)'\110',
    (CHAR)'\311', (CHAR)'\312', (CHAR)'\113', (CHAR)'\314', (CHAR)'\115',
    (CHAR)'\116', (CHAR)'\317', (CHAR)'\120', (CHAR)'\321', (CHAR)'\322',
    (CHAR)'\123', (CHAR)'\324', (CHAR)'\125', (CHAR)'\126', (CHAR)'\327',
    (CHAR)'\330', (CHAR)'\131', (CHAR)'\132', (CHAR)'\333', (CHAR)'\134',
    (CHAR)'\335', (CHAR)'\336', (CHAR)'\137', (CHAR)'\140', (CHAR)'\341',
    (CHAR)'\342', (CHAR)'\143', (CHAR)'\344', (CHAR)'\145', (CHAR)'\146',
    (CHAR)'\347', (CHAR)'\350', (CHAR)'\151', (CHAR)'\152', (CHAR)'\353',
    (CHAR)'\154', (CHAR)'\355', (CHAR)'\356', (CHAR)'\157', (CHAR)'\360',
    (CHAR)'\161', (CHAR)'\162', (CHAR)'\363', (CHAR)'\164', (CHAR)'\365',
    (CHAR)'\366', (CHAR)'\167', (CHAR)'\170', (CHAR)'\371', (CHAR)'\372',
    (CHAR)'\173', (CHAR)'\374', (CHAR)'\175', (CHAR)'\176', (CHAR)'\377'};

//  D O P A R  --  Add an appropriate parity bit to a character

CHAR dopar(register CHAR ch) {
  ...
  case 'e':
    return (p_tbl[a]); // Even
  case 'm':
    return ((CHAR)(a | 128)); // Mark
  case 'o':
    return ((CHAR)(p_tbl[a] ^ 128)); // Odd
  case 's':
    return ((CHAR)a); // Space
  default:
    return ((CHAR)a); // Something illegal
  }
}
...
long rttsamples, // Round trip time samples
    rttdelay,    // RTT delay
...
long crcta[16] = {0L,       010201L,  020402L,  030603L,  041004L,  051205L,
                  061406L,  071607L,  0102010L, 0112211L, 0122412L, 0132613L,
                  0143014L, 0153215L, 0163416L, 0173617L};

long crctb[16] = {0L,       010611L,  021422L,  031233L,  043044L,  053655L,
                  062466L,  072277L,  0106110L, 0116701L, 0127532L, 0137323L,
                  0145154L, 0155745L, 0164576L, 0174367L};
...
int input() {
  ...
  while (1) { // Big loop...
              // It is ttchk()'s responsibility to tell us if the connection is
              // broken, and to do so instantly and nondestructively -- no
              // blocking, etc, that would           slow down file transfer.
    if (ttchk() < 0) {
    ...
    if (nakstate) {         // This section for file receiver.
      ...
      if (type < -1) {
        char *s;
        s = (type == -2) ? "FAILED - Interrupted" : "FAILED - Connection lost";
...
// NAK the oldest as-yet-unNAK'd missing packet.
      ...
      if (y == 1) { // From previous window.
        ...
        // it is not the desired packet at winlo.  So therefore there are gaps
        // before this packet.  So we find the "lowest" unNAK'd missing packet,
        // if any, between winlo and this one, and NAK it.  If there are no
        // as-yet-unNAK'd missing packets in the window, then we send nothing
        // and go wait for another packet. In theory, this could result in a
        // timeout, but in practice it is likely that the already-NAK'd missing
        // packets are already on their way.  Note, we do not NAK ahead of
        // ourselves, as that only creates unnecessary retransmissions.
      ...
      if (!nak2ack) {                 // NAK(n+1) = ACK(n)
        if (wslots > 1) {             // Packet at winlo already ACK'd?
          if (sacktbl[winlo]) {       // If so,
            ...
            debug(F101, "input send returning pre-stashed ACK", "", winlo - 1);
            break;
          }
        }
#ifdef STREAMING
      ...
      if (type == 'Y') {      // Got an ACK
...
#endif // DEBUG
       // Slow start
              if (slostart && (maxsend <= spmax) && (rpackets < 11) &&
                  (numerrs == 0)) {
                spsiz = spsiz << 1;
                debug(F101, "bestlen spsiz A", "", spsiz);

                // Creep up to best length
              } else if ((spackets > 5) && (spsiz < bestlen - 8)) {
                spsiz += (bestlen - spsiz) / 3;
                debug(F101, "bestlen spsiz B", "", spsiz);

                // Push the envelope
              } else if ((spackets % (wslots + 1) == 0) && (spackets > 6) &&
                         (bestlen < spmax - 8) && (spsiz < spmax)) {
       ...
       // data, like if it's the ACK                          to an I, S, F, D,
       // Z, or just about any kind of packet.  So for now, freerbuf()
...
int parchk(CHAR *s, CHAR start, int n)
// parchk
{
  CHAR s0, s1, s2, s3;
...
int spack(char pkttyp, int n, int len, CHAR *d)
// spack
{
  register int i;
  ...
  if (longpkt) { // Long packet?
    ...
    chk = (unsigned)mydata[lp] + (unsigned)mydata[lp + 1] +
          (unsigned)mydata[lp + 2] + (unsigned)mydata[lp + 3] +
  ...
  // When sending a file, the data is already in the right place.  If it
  // weren't, it might make sense to optimize this section by using memcpy or
  // bcopy (neither of which are portable), but only if our packets were rather
  // long. When receiving, we're only sending ACKs so it doesn't matter.  So
  // count the following loop as a sleeping dog.
  ...
  if (x > -1) {
    ...
    // Sometimes on a congested TCP connection, it can surprise you -- 90
    // seconds or more...
    if (
#ifdef STREAMING
        !dontsend &&
...
int chk1(register CHAR *pkt, register int len) {
...
unsigned int chk2(register CHAR *pkt, register int len) {
  register long chk;
...
unsigned int chk3(register CHAR *pkt, register int len) {
  register long c, crc;
  for (crc = 0; len-- > 0; pkt++) {
    c = crc ^ (long)(*pkt);
...
int nxtpkt() { // Called by file sender
  int j, n, x;
...
int ack() { // Acknowledge the current packet.
  return (ackns(winlo, (CHAR *)""));
}

#ifdef STREAMING
int fastack() { // Acknowledge packet n
  int j, k, n, x;
  n = winlo;
...
int ackns(int n, CHAR *s) // Acknowledge packet n
{
  int j, k, x;
...
int ackn(int n) // Send ACK for packet number n
{
  return (ackns(n, (CHAR *)""));
}
...
// current packet is good, otherwise push a zero.  The current n-bit value, w,
// of this register is a weighted sum of the noise hits for the last n packets,
// with the most recent weighing the most.  The current packet length is some
// function of w and the negotiated packet length, like:
//
...
void rcalcpsz(void) {
  ...
  // in recpkt.
  int resend(int n) // Send packet n again.
  {
    int j, k, x;
#ifdef GFTIMER
    CKFLOAT t1 = 0.0, t2 = 0.0;
    ...
    if (k != 0 || j < 0) { // If not....
      if (nakstate && k == 1) {
        ...
        if (n == 0 && spackets < 63 &&
            myinit[0]) { // ACK to Send-Init
                         // If the packet number is 0, and we're at the
                         // beginning of a protocol operation (spackets < 63),
                         // then we have to resend the ACK to an I or S packet,
                         // complete with parameters in the data field.  So we
          ...
          if (bctf) { // Force Type 3 on all packets?
            ...
            bctl = bctlsav; // and length
                 ...
                 // It's not the first packet of the protocol operation.  It's
                 // some other packet        that we have already ACK'd and
                 // forgotten about. So we        take a chance and        send
                 // an empty ACK using the current        block-check type.
                 // Usually this will        work out OK (like when acking Data
                 // packets), and no great harm will be        done        if it
                 // was some other kind        of packet (F, etc).  If        we
                 // are requesting an        interruption of the        file
                 // transfer, the        flags are still set, so we'll catch up
                 // on        the        next        packet.
          x = spack('Y', n, 0, (CHAR *)"");
          if (x < 0) {
            return (x);
        ...
        // Packet number is not in current or previous window.  We seem to hit
        // this code occasionally at the beginning of a transaction, for
        // apparently no good reason.  Let's just log it for debugging, send
        // nothing, and try to proceed with the protocol rather than killing it.
    ...
    xxscreen(SCR_PT, '%', (long)pktnum,
             "(resend)"); // Tell user about resend
    logpkt('S', n, s_pkt[j].pk_adr,
           s_pkt[j].pk_len);  // Log the resent packet
    return (s_pkt[j].pk_rtr); // Return the number of retries.
  }

  //  E R R P K T  --  Send an Error Packet

  int errpkt(CHAR * reason) // ...containing the reason given
  {
    ...
    y = quiet;
    quiet = 1;
    epktsent = 1; // Close files silently.
    clsif();
    clsof(1);
    quiet = y;
...
// properly.
...
#endif // STREAMING
    if (what & W_RECV && (!server || (server && justone)) &&
        (wslots > 1
#ifdef STREAMING
         || streaming
#endif // STREAMING
  ...
  int srinit(int reget, int retrieve, int opkt) {
    int x = 0, left = 0;
...
#ifdef PARSENSE
#ifdef UNIX
      // So far the final turn argument is only for ck[uvdl]tio.c.  Should be
      // added to the others too.  (turn == handshake character.)
  ...
  i = 1; // ttinl does this for us
    ...
    if ((j = xunchar(recpkt[i++])) == 0) { // Get packet length.
      ...
      chk = (unsigned)((unsigned)recpkt[i - 1] + (unsigned)recpkt[i] +
                       (unsigned)recpkt[i + 1] + (unsigned)recpkt[i + 2] +
                       (unsigned)recpkt[i + 3]);
    ...
    // an echo and ignore it.  Don't even bother with the block check
    // calculation: even if the packet is corrupted, we don't want to NAK an
    // echoed packet. Nor must we NAK an ACK or NAK.
    ...
    case 3: // Type 3, 16-bit CRC
      crc =
          (xunchar(pbc[0]) << 12) | (xunchar(pbc[1]) << 6) | (xunchar(pbc[2]));
