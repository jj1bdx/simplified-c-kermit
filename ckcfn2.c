...
int input() {
  ...
  while (1) { /* Big loop... */
              /*
                ...
                and to do so instantly and nondestructively -- no blocking, etc, that
                would           slow down file transfer.
              */
    if (ttchk() < 0) {
...
*/
      ...
      if (y == 1) { /* From previous window. */
        ...
        /*
          ...
          as-yet-unNAK'd missing packets in the window, then we send nothing and
          go wait for another packet. In theory, this could result in a timeout,
          but in practice it is likely that the already-NAK'd missing packets
          are already on their way.  Note, we do not NAK ahead of ourselves, as
