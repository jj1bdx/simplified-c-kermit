...
static void dologdial(char *s) {
  ...
  m = strlen(uidbuf) + strlen(myhost) + strlen(ttname) + strlen(s) +
      strlen(buf2) + 32;
  if (n + m < CXLOGBUFL - 1) {
    p = cxlogbuf + n;
    if (diallcc && diallac) {
...
/* _dodial */ {
  ...
  if (!dialcmd &&                            /* Using default DIAL command */
    ...
    if (*telnbr != 'T' && *telnbr != 'P' && *telnbr != 't' && *telnbr != 'p' &&
        !ckstrcmp(s, "atd", 3, 0) && s[3] != 'T' && s[3] != 'P' &&
        s[3] != 't' && s[3] != 'p') {
      char xbuf[200];
...
xdialec:
  ...
  if (dialsta != DIA_PART) {
    ...
    if (x && xx_ok) { /* Look for OK response */
      ...
      if (x < 0) {
        ...
        printf(" Likely cause: Your modem is an RPI model, which does not have "
               "built-in\n");
    ...
    if (x) {
      ...
      if (x && mdmspk) {  /* Good response and speaker on? */
        ...
        case 1:
          ...
          break;
        case 2:
          s = dialvol2 ? dialvol2 : mp->vol2_str;
          break;
        case 3:
          s = dialvol3 ? dialvol3 : mp->vol3_str;
          break;
        default:
          s = NULL;
        }
    ...
    if (mdmcapas & CKD_AT) { /* Hayes AT command set */
      ...
      case n_ATTMODEM:
        ...
        if (didweget(lbuf, "Busy")) {
          ...
          mdmstat = D_FAILED;
          dialsta = DIA_NODT;
        } else if (didweget(lbuf, "No answer") || didweget(lbuf, "No Answer")) {
          mdmstat = D_FAILED;
          dialsta = DIA_NOAN;
        } else if (didweget(lbuf, "Answered") || didweget(lbuf, "Connected")) {
          mdmstat = CONNECTED;
          dialsta = DIA_OK;
        }
        break;
...
#ifdef OLDMODEMS
      case n_VENTEL:
        if (didweget(lbuf, "ONLINE!") || didweget(lbuf, "Online!")) {
          mdmstat = CONNECTED;
        } else if (didweget(lbuf, "BUSY") || didweget(lbuf, "Busy")) {
          mdmstat = D_FAILED;
          dialsta = DIA_BUSY;
  ...
  if (!quiet && !backgrd) {
    if (dialdpy && (p = ck_time())) {  /* If DIAL DISPLAY ON, */
      ...
      printf(" %sall complete: \"%s\".\n",
             (mdmstat == D_PARTIAL) ? "Partial c" : "C", (char *)modemmsg);
    } else {
    ...
    if (x1 == 0 && x2 == 0 && dialsta != DIA_PART) {
      ...
      if (hints && !quiet && !network && spdmax > 0L && speed > spdmax
        ...
        printf("Interface speed %ld might be too high for this modem type.\n",
               speed);
        printf("If dialing fails, SET SPEED to %ld or less and try again.\n",
               spdmax);
    ...
    */
    ...
    case 33: /* FAX connection */
      if (is_rockwell || is_supra || mymdmtyp == n_ZOLTRIX ||
        ...
        mdmstat = D_FAILED;
        dialsta = DIA_FAX;
      } else if (mymdmtyp == n_UCOM_AT || is_motorola ||
