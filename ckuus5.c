...
extern int carrier, cdtimo, local, quiet, backgrd, bgset, sosi, xsuspend,
    ...
    diractive, tlevel, cwdf, nfuncs, msgflg, remappd, hints, mdmtyp, zincnt,
    cmask, rcflag, success, xitsta, pflag, tnlm, tn_nlm, xitwarn, debses,
    xaskmore, parity, saveask, wasclosed, whyclosed, cdactive, rcdactive,
    keepallchars, cmd_err;
...
extern char *protv, *fnsv, *cmdv, *userv, *ckxv, *ckzv, *ckzsys, *xlav, *cknetv,
    *clcmds;
...
#ifndef NOXFER
extern int displa, bye_active, protocol, pktlog, remfile, rempipe, unkcs, keep,
    lf_opts, fncnv, pktpaus, autodl, xfrcan, xfrchr, xfrnum, srvtim, srvdis,
    query, retrans, streamed, reliable, crunched, timeouts, fnrpath, autopath,
    rpackets, spackets, epktrcvd, srvping;
...
char *for_def[] = {
    ...
    "\\fcontents(\\%6),:_..inc,incr \\\\\\%1 \\%4,goto "
    "_..top,:_..bot,_putargs},",
    "define break goto _..bot, define continue goto _..inc,",
    "do _for\\v(cmdlevel) \\\\%1 \\%2 \\%3 \\%4 { \\%5 },_assign "
    "_for\\v(cmdlevel)",
    ""};
...
int *ifcmd = NULL, *count = NULL, *iftest = NULL, *intime = NULL,
    *inpcas = NULL, *takerr = NULL, *merror = NULL;
#else
  ...
  */
  ...
  if (!k_info_dir) {
    ...
    for (i = 0; *(txtdir[i]); i++) {
      ...
      ckmakmsg(xbuf, CKMAXPATH + 32, txtdir[i], "kermit/", "ckubwr.txt", NULL);
      if (zchki(xbuf) > 0) {
      ...
      ckmakmsg(xbuf, CKMAXPATH + 32, txtdir[i], "ckermit/", "ckubwr.txt", NULL);
      if (zchki(xbuf) > 0) {
...
int parser(int m) {
  ...
  if (sndcmd && protocol == PROTO_K && !success && hints && !interrupted &&
      !fatalio && !xcmdsrc) {
    int x = 0, n = 0;
    printf("\n*************************\n");
    ...
    printf("\nHINTS... If the preceding error message%s not explain the "
           "failure:\n",
           (n > 1) ? "s do" : " does");
#ifndef NOLOCAL
    if (local) {
      if (rpackets == 0) {
        ...
        printf(" . Try changing the remote Kermit's FLOW-CONTROL setting.\n");
        if (!network && mdmtyp > 0)
          if ((3 * crunched) > spackets)
            printf(" . Try placing a new call to get a cleaner connection.\n");
      }
    } else if (rpackets > 0) {
      if (flow == FLO_NONE)
  ...
  if (getcmd && protocol == PROTO_K && !success && hints && !interrupted &&
      !fatalio && !xcmdsrc) {
  ...
  while (sstate == 0) { /* Parse cmds until action requested */
    ...
    if ((local && !xcmdsrc && is_tn && TELOPT_ME(TELOPT_KERMIT) &&
        ...
        (!local && !cmdadl && TELOPT_ME(TELOPT_KERMIT) &&
         TELOPT_SB(TELOPT_KERMIT).kermit.me_start)) {
      tn_siks(KERMIT_STOP);
    }
#endif /* IKS_OPTION */
    ...
    if (ttchk() < 0) {
      if (!network) {
        if (carrier != CAR_OFF) {
          ...
          x = ttgmdm();
          if ((x > -1) && ((x & BM_DCD) == 0)) {
            printf("?Carrier signal required but not present - Try SET "
                   "CARRIER-WATCH OFF.\n");
            return (0);
          }
        } else {
          printf("?Problem with serial port or modem or cable - Try SHOW "
                 "COMMUNICATIONS.\n");
          return (0);
        }
      }
      printf("?Connection %s %s is not open or not functioning.\n",
             network ? "to" : "on", ttname);
      return (0);
    }
...
int addmac(char *nam, char *def) /* Add a macro to the macro table */
{
  ...
  if (inserver &&
    ...
    if (!ckstrcmp("on_exit", nam, -1, 0) || !ckstrcmp("on_logout", nam, -1, 0))
      return (-1);
  }
#endif /* IKSD */
...
int delmac(char *nam, int exact) /* Delete the named macro */
{
  ...
  if (*nam == '&') { /* An array reference? */
    ...
    if (q[z]) {                    /* If there is an old value, */
      ...
      debug(F010, "delmac def", "(null pointer)", 0);
  }
...
int doclslog(int x) {
  ...
  case LOGM:
    if (dialog <= 0) {
      if (msgflg)
        printf("?Connection log wasn't open\n");
      return (0);
    }
    *diafil = '\0';
    dialog = 0;
...
char *showoff(int x) { return (x ? "on" : "off"); }
...
VOID shotcs(int csl, int csr) /* Show terminal character set */
{
  ...
  if (tcs_transp) { /* No translation */
    ...
    if (csr != csl) {
      ...
      if (strcmp(s, fcsinfo[csl].keyword) && strcmp(s, fcsinfo[csr].keyword))
        printf("                         (via %s)\n", s);
    }
  }
#endif /* NOCSETS */
...
VOID shotrm() {
  ...
  printf(" %19s: %1d%-12s  %13s: %1d%-14s\n", "Bytesize: Command",
         (cmdmsk == 0377) ? 8 : 7, " bits", "Terminal", (cmask == 0377) ? 8 : 7,
         " bits");
  s = getenv("TERM");
#ifdef XPRINT
  printf(" %19s: %-13s  %13s: %-15s\n", "Type", s ? s : "(unknown)", "Print",
         showoff(tt_print));
#else
  printf(" %19s: %-13s\n", "Type", s ? s : "(unknown)");
#endif /* XPRINT */
  ...
  if (apcstatus == APC_ON)
    s = "on";
  else if (apcstatus == APC_OFF)
    s = "off";
  else if (apcstatus == (APC_ON | APC_UNCH))
    s = "unchecked";
  else if (apcstatus == (APC_ON | APC_NOINP))
    s = "no-input";
...
void shotapi(int option) {
...
#ifdef TAPI_BETATEST
    if (tapipass) {
      printf("K-95 uses the TAPI Line in an exclusive mode.  Other "
             "applications\n");
      printf("may open the device but may not place calls nor answer calls.\n");
      printf("Dialing is performed using the K-95 dialing procedures.  SET "
             "MODEM\n");
      printf(
          "TYPE TAPI after the SET TAPI LINE command to activate the modem\n");
      printf("definition associated with the active TAPI LINE device.\n\n");

    } else {

      printf("K-95 uses the TAPI Line in a cooperative mode.  Other "
             "applications\n");
      printf("may open the device, place and answer calls.  Dialing is "
             "performed\n");
      printf("by TAPI.  K-95 SET MODEM commands are not used.\n\n");
    }

    if (tapiconv == CK_ON || tapiconv == CK_AUTO && !tapipass) {
      printf("Phone numbers are converted from canonical to dialable form by "
             "TAPI\n");
      printf(
          "using the dialing rules specified in the TAPI Dialing Properties\n");
      printf("dialog.\n\n");
      ...
      printf("Phone numbers are converted from canonical to dialable form by "
             "K-95\n");
      printf("using the dialing rules specified with the SET DIAL commands.  "
             "TAPI\n");
      printf("Dialing Properties are imported automaticly upon startup and "
             "whenever\n");
      printf("the TAPI Dialing Properties are altered or when the TAPI "
             "Location\n");
      printf("is changed.\n\n");
    }
#endif /* TAPI_BETATEST */
  ...
  case 1:
    ...
    break;
  case 2:
    rc = cktapiGetModemSettings(&lpDevCfg, &lpModemSettings, &lpCommConfig,
                                &lpDCB);
    if (rc) {
...
int showarray() {
  ...
  if (*s) {
    ...
    if (!ap) {
      ...
      int i, n, max;
      max = (range[1] > 0) ? range[1] : ((range[0] > 0) ? range[0] : a_dim[x]);
      if (range[0] < 0)
        range[0] = 0;
      if (max > a_dim[x])
        max = a_dim[x];
  ...
  for (y = 0; y < (int)'z' - ARRAYBASE + 1; y++) {
    if (a_ptr[y]) {
      ...
      x = 1;
      printf(" \\&%c[%d]", (y == 1) ? 64 : y + ARRAYBASE, a_dim[y]);
      if (a_link[y] > -1)
        printf(" => \\&%c[]", arrayitoa(a_link[y]));
      printf("\n");
    }
    if (!x)
      printf(" No arrays declared\n");
  }
  return (1);
}
#endif /* NOSPL */
...
int doshow(int x) {
  ...
  if (x == SHMAC) { /* SHOW MACRO */
    ...
    if (n == 0) {
      ...
      slc = 1;
      for (y = 0; y < nmac; y++)
        if (shomac(mactab[y].kwd, mactab[y].mval) < 0)
          break;
      return (1);
    }
    slc = 0;
...
*/
  ...
  case SHEXI: {
    ...
    printf("\n Exit warning %s\n",
           xitwarn ? (xitwarn == 1 ? "on" : "always") : "off");
    printf(" Exit message: %s\n",
           exitmsg ? (exitmsg == 1 ? "on" : "stderr") : "off");
  ...
  case SHPRT: {
    ...
    printf("Printer: %s%s\n",
           ...
           printpipe ? "| " : "", printername ? printername : "(default)");
#ifdef PRINTSWI
#ifdef BPRINT
    if (printbidi) {
  ...
  case SHOSTR: { /* STREAMING */
    ...
    printf(" Streaming:    %s\n\n", showooa(streamrq));
    if ((!local && (streamrq == SET_ON)) || (streamrq == SET_AUTO && reliable))
      ...
      printf(" Streaming will be requested and will be done if the other "
             "Kermit agrees.\n");
    printf(" Last transfer: %sstreaming%s, %ld cps.\n",
           streamed > 0 ? "" : "no ", cleared ? ", clearchannel" : "", tfcps);
    printf("\n");
    break;
  }
#endif /* STREAMING */
...
int litcmd(char **src, char **dest, int n) {
  int bc = 0, pp = 0;
  char c, *s, *lp, *ss;
...
int prtopt(int *lines, char *s) /* Print an option */
{
  ...
  if (hpos > ((tt_cols > 40) ? (tt_cols - 1) : 79)) {
    printf("\n");
    if (++i > (cmd_rows - 3)) {
      if (!askmore())
        return (0);
