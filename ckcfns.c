...
/* xgnbyte */ {
  ...
  case XLA_UNICODE: { /* Unicode is involved */
    ...
    /*
      ...
      called by getpkt() (i.e. when transferring a file), we are translating
      from Unicode to the Transfer Character Set and therefore must use the
      function pointed to by xut.  Otherwise, e.g. during TRANSLATE, CONNECT,
      TRANSMIT, etc, we are translating from Unicode to the File Character Set
      and so must call the function pointed to by xuf.  There might be a cleaner
      way to set this up but I don't think so.  For example, setxlatype() might
      have been called too soon and so might not have known whether it was a
      file transfer or a local operation.
    */
    /*
      ...
      file conversion.  In the case of TRANSLATE, we call (xgnbyte(), xpnbyte())
      in a loop, expecting the xgnbyte() will feed UCS2 to xpnbyte().  But the
      following code does what xpnbyte() is going to do, returning (in this
      case) an ISO Latin/Cyrillic byte stream, which xpnbyte() believes to be
      UCS2, and comes up with nonsense.  Not wanting to rip the whole thing
      apart and start over, I made the following change that should do no harm,
      upon observing that if the input character set is UTF-8 or UCS-2, then
      when we get here it has already been converted to UCS2, so if we are not
      transferring a file, we don't need to do anything else except put the
      bytes in the right place to be returned, which is done further along.
    */
    /* New code 2011-06-03 */
    if (what & W_SEND) {
      xx = xut;
    ...
    */
    ...
    if (rt & 0200) {                   /* 8-bit character? */
      ...
      /*
        ...
        character's 8th bit will be destroyed in transmission, and a block check
        error will occur.
      */
