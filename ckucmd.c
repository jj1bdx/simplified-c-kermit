...
struct keytab cmonths[] = {
    {"april", 4, 0},     {"august", 8, 0},   {"december", 12, 0},
    {"february", 2, 0},  {"january", 1, 0},  {"july", 7, 0},
    {"june", 6, 0},      {"march", 3, 0},    {"may", 5, 0},
    {"november", 11, 0}, {"october", 10, 0}, {"september", 9, 0}};
...
// . settable prompt, protected from deletion, dynamically re-evaluated each
// time . allows chained parse functions.
//
// Functions:
...
//
// Before using these routines, the caller should #include "ckucmd.h" and set
// the program's prompt by calling cmsetp().  If the file parsing functions
// cmifi, cmofi, or cmdir are to be used, this module must be linked with a
// ck?fio file system support module for the appropriate system, e.g. ckufio for
// Unix.  If the caller puts the terminal in character wakeup ("cbreak") mode
// with no echo, then these functions will provide line editing -- character,
// word, and line deletion, as well as keyword and filename completion upon ESC
// and help strings, keyword, or file menus upon '?'.  If the caller puts the
// terminal into character wakeup/noecho mode, care should be taken to restore
// it before exit from or interruption of the program.  If the character wakeup
// mode is not set, the system's own line editor may be used.
//
...
// keyboard, erase character from screen, etc) should eventually be turned into
// a call to a function that is defined in ck?tio.c, but then all the ck?tio.c
...
static int psetf = 0, // Flag that prompt has been set
...
static int current = -1,     // Pointer to current command
...
// kwdhelp
{
  ...
  if (off < 0) {
    off = 0; // Offset for first page
  }
  if (!pre) {
    pre = ""; // Handle null string pointers
  }
  if (!post) {
    post = "";
  }
  ...
  if ((s2 = (char **)malloc(n * sizeof(char *)))) {
    for (i = 0; i < n; i++) { // Find longest keyword
      ...
      if (s[i].flgs & CM_INV) {
        // This code shows invisible keywords that are not also abbreviations
        // when
        // ? was typed AFTER the beginning of the field so the user can find out
        // what they are and (for example) why completion doesn't work at this
        // point
      ...
      if (!(xhlp & 4) || !tmpbuf) { // Regular keyword table
        ...
        ckmakmsg(tmpbuf,            // Make a copy that shows ":" if
                 ...
                 s[i].kwd, (s[i].flgs & CM_ARG) ? ":" : "", NULL, NULL);
  ...
  if (s2 && (b = (char *)malloc(cmd_cols + 1))) { // Make a line buffer
    ...
    if (n % cols) {
      height++; // Add one for remainder, if any
    }
...
int xfilhelp(int n, char *pre, char *post, int off, int cmdirflag, int fs,
             char *sa, char *sb, char *sna, char *snb, CK_OFF_T minsiz,
             CK_OFF_T maxsiz, int nbu, int nxlist, char **xlist) {
  ...
  if ((s2 = (char **)malloc(n * sizeof(char *)))) {
    for (i = 0; i < n; i++) { // Loop through filenames
      ...
      if (fs) {
        if (fileselect(filbuf, sa, sb, sna, snb, minsiz, maxsiz, nbu, nxlist,
                       xlist) < 1) {
          continue;
        }
      }
...
int filhelp(int n, char *pre, char *post, int off, int cmdirflg) {
  // filhelp
  return (xfilhelp(n, pre, post, off, cmdirflg, 0, NULL, NULL, NULL, NULL,
...
#ifdef DCMDBUF
int cmsetup() {
  if (!(cmdbuf = malloc(CMDBL + 4))) {
    return (-1);
  }
  if (!(savbuf = malloc(CMDBL + 4))) {
    return (-1);
  }
  savbuf[0] = '\0';
  if (!(prevcmd = malloc(CMDBL + 4))) {
    return (-1);
  }
  prevcmd[0] = '\0';
  if (!(atmbuf = malloc(ATMBL + 4))) {
    return (-1);
  }
  if (!(atxbuf = malloc(CMDBL + 4))) {
    return (-1);
  }
  if (!(atybuf = malloc(ATMBL + 4))) {
    return (-1);
  }
  if (!(filbuf = malloc(ATMBL + 4))) {
    return (-1);
  }
  if (!(cmprom = malloc(PROMPTL + 4))) {
    return (-1);
  }
  if (!(cmprxx = malloc(PROMPTL + 4))) {
    return (-1);
  }
#ifdef CK_RECALL
  cmrini(cm_recall);
...
char *cmgetp() { return (cmprxx); }

int cmgbrk() { return (brkchar); }

int cmgkwflgs() { return (cmkwflgs); }
...
// cmini
  ...
  for (bp = atmbuf; bp < atmbuf + ATMBL; bp++) {
    *bp = NUL;
  }
#endif // USE_MEMCPY
...
int cmpush() { // Save the command environment
  ...
  if (!cmp &&
      !(cmp = (struct cmp *)malloc(sizeof(struct cmp) * (CMDDEP + 1)))) {
...
int cmpop() { // Restore the command environment
  if (cmddep < 0) {
...
int cmnum(char *xhlp, char *xdef, int radix, int *n, xx_strp f) {
...
// fdc, 24 Dec 2005.
int cmnumw(char *xhlp, char *xdef, int radix, CK_OFF_T *n, xx_strp f) {
  // cmnumw
  int x;
  char *s, *zp, *zq;
  ...
  if (!*zp) {
    zp = xdef; // Result empty, substitute default
  }
...
int cmofi(char *xhlp, char *xdef, char **xp, xx_strp f) {
  int x;
  char *s, *zq;
#ifdef DOCHKVAR
  int tries;
...
// least try making cmofi() symmetrical with cmifi()...
  ...
  if ((strcmp(s, CTTNAM) == 0) &&
      (zchko(s) <
       0)) { // write to console OK
             // Note: there are certain circumstances where zchko() can give a
             // false positive, so don't rely on it to catch every conceivable
             // situation in which the given output file can't be created.  In
             // other words, we print a message and fail here if we KNOW the
             // file can't be created.  If we succeed but the file can't be
             // opened, the code that tries to open the file has to print a
             // message.
...
int cmifi(char *xhlp, char *xdef, char **xp, int *wild, xx_strp f) {
...
int cmifip(char *xhlp, char *xdef, char **xp, int *wild, int d, char *path,
           xx_strp f) {
...
int cmdir(char *xhlp, char *xdef, char **xp, xx_strp f) {
  int wild;
...
int cmdirp(char *xhlp, char *xdef, char **xp, char *path, xx_strp f) {
  int wild;
...
int cmifi2(char *xhlp, char *xdef, char **xp, int *wild, int d, char *path,
           xx_strp f, int dirflg)
// cmifi2
{
  ...
  while (1) {
    ...
    case 0:          // SP
      ...
      *xp = brstrip(*xp);         // Strip braces
      ...
      // This is to get around "cd /" failing because "too many directories
      // match"
      ...
      *wild = (iswild(sv) || (y > 1)) && (itsadir == 0);
      ...
      if (itsadir && d && !dirflg) { // It's a directory and not wild
        if (sv) {
          free(sv); // and it's ok to parse directories
        }
        if (np) {
          free(np);
        }
        ...
        if (dosearch) { // Search the path...
          ...
          while (1) {
            ...
            if (c == PATHSEP || c == NUL) {
              ...
              *ptr = NUL;
#ifdef UNIX
              // By definition of CDPATH, an empty member denotes the current
              // directory
              if (!*path) {
      ...
      *wild = (y > 1);
      if (y == 0) {
        ...
        if (y == 1 && isdir(filbuf)) { // Dont we already know this?
          ...
          // At this point, before just doing partial completion, we should look
          // first to see if the given directory does indeed have any
          // subdirectories (dirflg) or files (!dirflg); if it doesn't we should
          // do full completion.  Otherwise, the result looks funny to the user
          // and "?" blows up the command for no good reason.
          {
            int flags = 0;
            filbuf[len + 1] = '*';
            filbuf[len + 2] = NUL;
...
int cmfld(char *xhlp, char *xdef, char **xp, xx_strp f) {
  int x, xc, isavar = 0;
  char *zq;
...
int cmtxt(char *xhlp, char *xdef, char **xp, xx_strp f) {
  int x, i;
  char *xx, *zq;
  static int xc;
...
// cmswi()  - Switch parser
int cmkey(struct keytab table[], int n, char *xhlp, char *xdef, xx_strp f) {
  // cmkey
  return (cmkey2(table, n, xhlp, xdef, "", f, 1));
}
int cmkeyx(struct keytab table[], int n, char *xhlp, char *xdef, xx_strp f)
// cmkeyx
{
  return (cmkey2(table, n, xhlp, xdef, "", f, 0));
}
int cmswi(struct keytab table[], int n, char *xhlp, char *xdef, xx_strp f) {
  // cmswi
  return (cmkey2(table, n, xhlp, xdef, "", f, 4));
}
// c m k e y 2 -- The normal keyword parsing function
int cmkey2(struct keytab table[], int n, char *xhlp, char *xdef, char *tok,
           xx_strp f, int pmsg)
// cmkey2
{
  ...
  while (1) {
    ...
    case 2: // User terminated word with ESC
      ...
      // See if the keyword just found has the CM_ABR bit set in its flgs field,
      // and if so, search forwards in the table for a keyword that has the same
      // kwval but does not have CM_ABR (or CM_INV?) set, and then expand using
      // the full keyword.  WARNING: This assumes that (a) keywords are in
      // alphabetical order, and (b) the CM_ABR bit is set only if the the
      // abbreviated keyword is a true abbreviation (left substring) of the full
      // keyword.
...
static struct keytab timeunits[] = {
    {"days", TU_DAYS, 0}, {"months", TU_MONTHS, 0}, {"weeks", TU_WEEKS, 0},
    {"wks", TU_WEEKS, 0}, {"years", TU_YEARS, 0},   {"yrs", TU_YEARS, 0}};
...
static struct keytab symdaytab[] = {{"now", SYM_NOW, 0},
                                    {"today", SYM_TODA, 0},
                                    {"tomorrow", SYM_TOMO, 0},
...
static struct keytab daysofweek[] = {
    {"Friday", 5, 0},   {"Monday", 1, 0},  {"Saturday", 6, 0}, {"Sunday", 0, 0},
    {"Thursday", 4, 0}, {"Tuesday", 2, 0}, {"Wednesday", 3, 0}};

static struct keytab usatz[] = { // RFC 822 timezones
    {"cdt", 5, 0},               // Values are GMT offsets
    {"cst", 6, 0}, {"edt", 4, 0}, {"est", 5, 0}, {"gmt", 0, 0}, {"mdt", 6, 0},
    {"mst", 7, 0}, {"pdt", 7, 0}, {"pst", 8, 0}, {"utc", 0, 0}, {"zulu", 0, 0}};
...
// asctime pattern
static char *atp1 = "[A-Z][a-z][a-z] [A-Z][a-z][a-z] [ 0-9][0-9] "
                    "[0-9][0-9]:[0-9][0-9]:[0-9][0-9] [0-9][0-9][0-9][0-9]";

// asctime pattern with timezone
static char *atp2 =
    "[A-Z][a-z][a-z] [A-Z][a-z][a-z] [ 0-9][0-9] "
    "[0-9][0-9]:[0-9][0-9]:[0-9][0-9] [A-Z][A-Z][A-Z] [0-9][0-9][0-9][0-9]";
...
char *cmdelta(int yy, int mo, int dd, int hh, int mm, int ss, int sign, int dyy,
              int dmo, int ddd, int dhh, int dmm, int dss) {
  // cmdelta
  long t1, t2, t3, t4;
  long d1 = 0, d2;
...
int delta2sec(char *s, long *result) {
  long ddays = 0L, zz;
    ...
    if (isdatesep(*p)) { // Have a date separator
      ...
      *p++ = NUL; // Replace by NUL
      if (*p) {   // Now we're at the next field
        ...
        if (!*p) {
          break; // Make sure we still have something
        }
        if (i == 2) { // Last one?
          break;
        }
...
dotime:
  ...
  sprintf(yyyymmdd, "%s%s%02d", year, month,
          (char)nday); // for tz calculations...
...
xcmdate:
  ...
  if ((mm < 0 || ss < 0) || (t != 2 && (ss > 0 || mm > 0) && hh > 23)) {
...
#endif // DEBLOG
    if (!(dp = cmdelta(atoi(year), atoi(month), nday, hh, mm, ss, dsign, dyears,
                       dmonths, ddays, dhours, dmins, dsecs))) {
      debug(F111, "cmcvtdate", cmdatemsg, -1);
      return (NULL);
    }
  }
...
int cmvdate(char *d) // Verify date-time
{
  int i;
  if (!d) {
    return (0);
  ...
  if (!isdigit(d[9]) || !isdigit(d[10]) || !isdigit(d[12]) || !isdigit(d[13]) ||
...
char *cmdiffdate(char *d1, char *d2) {
...
char *shuffledate(char *p, int opt) {
  extern char *wkdays[];
  int len;
  char ibuf[32];
...
#define MONTHBUFLEN 32
  ...
  if (!nolocale) { // If user didn't do --nolocale
    ...
    if (opt == 1 || opt == 2) {        // Short month name
      ...
      if (!s) {
        s = fullmonthname[mm - 1]; // Allow for error
      }
...
int cmdate(char *xhlp, char *xdef, char **xp, int quiet, xx_strp f) {
  int x, rc;
  char *o, *s, *zq, *dp;
  ...
  *xp = "";
  ...
  if (f) { // If a conversion function is given
    ...
    if (setatm(pp, 0) < 0) {
      if (!quiet) {
        printf("?Evaluated date too long\n");
      }
      return (-9);
    }
    s = atxbuf;
...
int savhistory(char *s, int disp) {
  FILE *fp;
  int i;
...
struct OFDB cmresult = {
    // Universal cmfdb result holder
...
int cmfdb(struct FDB *fdbin) {
  ...
  while (1) { // Loop through the chain of FDBs
    ...
    case _CMOFI:
      ...
      break;
    case _CMIFI:
      x = cmifi2(in->hlpmsg, in->dflt, &s, &n, in->ndata1, in->sdata, in->spf,
                 in->ndata2);
    ...
    case _CMTXT:
      ...
      break;
    case _CMKEY:
      x = cmkey2(in->kwdtbl, in->ndata1, in->hlpmsg, in->dflt, in->sdata,
                 in->spf, in->ndata2);
      debug(F101, "cmfdb cmkey", "", x);
      if (x < 0) {
        errbits |= ((in->ndata2 & 4) ? 32 : 64);
      }
      break;
    case _CMCFM:
      x = cmcfm();
    ...
    if (!in) { // No more
      ...
      if (x == -9) {
       ...
       // Make informative messages for a few common cases
          switch (errbits) {
          case 4 + 32:
            m = "Does not match filename or switch";
            break;
          case 4 + 64:
            m = "Does not match filename or keyword";
            break;
          case 1 + 32:
            m = "Not a number or valid keyword";
            break;
          case 1 + 64:
            m = "Not a number or valid switch";
            break;
          default:
            m = "Not valid in this position";
          }
        printf("?%s: \"%s\"\n", m, atmbuf);
      }
      return (x);
...
int cmiofi(char *xhlp, char *xdef, char **xp, int *wild, xx_strp f) {
  int x;
  struct FDB f1, f2;
...
// versions.  Add Zmodem later...
...
#ifdef EINTR
        // Some operating and/or C runtime systems return EINTR for no good
        // reason, when the end of the standard input "file" is encountered.  In
        // cases like this, we get into an infinite loop; hence the eintr
        // counter, which is reset to 0 upon each call to this routine.
    ...
    if (quote == 0) { // If this is not a quoted character
      ...
      if (c == LF || c == CK_CR) { // CR or LF.
        ...
        iscontd = prevchar != CMDQ && (lastchar == '-' || lastchar == lbrace);
      ...
      // This section handles interactive help, completion, editing, and
      // history. Rearranged as a switch statement executed only if we're at top
      // level since there is no need for any of this within command files and
      // macros: Aug 2000. Jun 2001: Even if at top level, skip this if the
      // character was fetched from the reparse or recall buffer, or if stdin is
      // redirected.
...
#endif // NOSPL
          if (quoting && !kstartactive && !comment) {
            cmdecho((char)c, 0);
            *bp = NUL;
            if (setatm(pp, 0) < 0) {
      ...
      if (c > 32 && c != '?' && c != RUB && chsrc != 0) {
        ...
        *bp++ = CMDQ;  // Deposit \ if it came from tty
        ...
        linebegin = 0; // Not at beginning of line
#ifdef BS_DIRSEP
        // This is a hack to handle "cd \" or "cd foo\" on OS/2 and similar
        // systems. If we were called from cmdir() and the previous character
        // was the quote character, i.e. backslash, and this character is the
        // command terminator, then we stuff an extra backslash into the buffer
        // without echoing, then we stuff the carriage return back in again, and
        // go back and process it, this time with the quote flag off.
      ...
      if (echof) {
        cmdecho((char)c, qf); // Now echo quoted character
      }
    ...
    if (quote) {
      continue; // Don't deposit quote character.
    }
...
static int setatm(char *cp, int fcode) {
...
//  printf("CP=[%s]\n",cp);
  ...
  if (cp == ap) { // In case source is atom buffer
    ...
    s = ap;
    while ((*xp++ = *s++))
      ; // We already know it's big enough
    cp = xp = atybuf;
  }
...
int cmdgetc(int timelimit) // Get a character from the tty.
{
  int c;
#ifdef IKSD
  extern int inserver;
...
static void cmdclrscn() { // Clear the screen
  ck_cls();
}
...
int xxesc(char **s) // Expand backslash escapes
{
  ...
  if (x != CMDQ) {
    return (-1); // make sure it's a backslash code
  }
...
int nlookup(struct keytab table[], char *word, int n, int *x) {
  register int i;
  ...
  while (*s++) {
    wordlen++; // Length of word to look up
  }
  ...
  if (i > n) {
    i = 0; // Loop exhausted makes i too large by one
  }
...
int xlookup(struct keytab table[], char *cmd, int n, int *x) {
  register int i;
...
char *rlookup(struct keytab table[], int n, int x) {
  int i;
  for (i = 0; i < n; i++) {
    if (table[i].kwval == x) {
      return (table[i].kwd);
