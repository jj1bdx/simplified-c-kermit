...
#define TELCMD(x)                                                              \
  (TELCMD_OK(x) ? ((x) == TN_SAK ? "SAK" : telcmds[(x) - TELCMD_FIRST])        \
                : tel_unk(x))
...
#ifdef TELOPT_MACRO
#define TELOPT_INDEX(x)                                                         \
  (((x) >= 0 && (x) <= TELOPT_STDERR)                               ? (x)       \
   : ((x) >= TELOPT_PRAGMA_LOGON && (x) <= TELOPT_PRAGMA_HEARTBEAT) ? (x) - 87  \
   : ((x) == TELOPT_IBM_SAK)                                        ? (x) - 146 \
                                                                    : NTELOPTS)

#define TELOPT_OK(x)                                                           \
  (((x) >= TELOPT_BINARY && (x) <= TELOPT_STDERR) ||                           \
...
char *telopts[NTELOPTS + 2] = {
    ...
    //  36
    "OLD-ENVIRON", "AUTHENTICATION", "ENCRYPTION", "NEW-ENVIRONMENT",
    //  40
    "TN3270E", "xauth", "CHARSET", "remote-serial-port",
    //  44
    "COM-PORT-CONTROL", "suppress-local-echo", "START-TLS", "KERMIT",
    //  48
    "send-url", "FORWARD-X", "stderr",
    // 138
    "pragma-logon", "sspi-logon", "pragma-heartbeat",
    // 200
    "ibm-sak", "unknown", 0};
#else  // TELOPTS
extern char *telopts[];
...
#define TELOPT_ME_MODE(x) (telopt_states[TELOPT_INDEX(x)].me_mode)
#define TELOPT_U_MODE(x) (telopt_states[TELOPT_INDEX(x)].u_mode)
#define TELOPT_UNANSWERED_WILL(x)                                              \
  (telopt_states[TELOPT_INDEX(x)].unanswered_will)
#define TELOPT_UNANSWERED_DO(x) (telopt_states[TELOPT_INDEX(x)].unanswered_do)
...
#define TELOPT_UNANSWERED_SB(x) (telopt_states[TELOPT_INDEX(x)].unanswered_sb)
#define TELOPT_SB(x) (telopt_states[TELOPT_INDEX(x)].sb)
...
#ifdef TELOPT_MODES
char *telopt_modes[4] = {"REFUSED", "ACCEPTED", "REQUESTED", "REQUIRED"};
#else  // TELOPT_MODES
...
#define SLC_NAMELIST                                                           \
  "0", "SYNCH", "BRK", "IP", "AO", "AYT", "EOR", "ABORT", "EOF", "SUSP", "EC", \
      "EL", "EW", "RP", "LNEXT", "XON", "XOFF", "FORW1", "FORW2", "MCL",       \
      "MCR", "MCWL", "MCWR", "MCBOL", "MCEOL", "INSRT", "OVER", "ECR", "EWR",  \
      "EBOL", "EEOL", 0
#ifdef SLC_NAMES
...
#ifdef AUTH_NAMES
char *authtype_names[] = {"NULL",         // RFC 2941
...
#ifdef TNC_NAMES
char *tnc_names[] = {"SIGNATURE",           "SET-BAUDRATE",
                     "SET-DATARATE",        "SET-PARITY",
                     "SET-STOPSIZE",        "SET-CONTROL",
                     "NOTIFY-LINESTATE",    "NOTIFY-MODEMSTATE",
                     "FLOWCONTROL-SUSPEND", "FLOWCONTROL-RESUME",
                     "SET-LINESTATE-MASK",  "SET-MODEMSTATE-MASK",
                     "PURGE-DATA",          0};
#else
