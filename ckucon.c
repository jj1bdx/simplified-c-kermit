...
extern int local, escape, duplex, parity, flow, seslog, sessft, debses, mdmtyp,
    ttnproto, cmask, cmdmsk, network, nettype, deblog, sosi, tnlm, xitsta, what,
    ttyfd, ttpipe, quiet, backgrd, pflag, tt_crd, tt_lfd, tn_nlm, ttfdflg,
...
static int pipemsg(n)
int n;
{ // Send message ID to parent
  int code = n & 255;
...
static void ck_sndmsg() { // Executed by child only ...
  ...
  if (
    ...
    for (;;) {
      pause(); // Wait for CK_FORK_SIG or SIGKILL
    }
    // NOTREACHED
  }
...
#endif // NOICP
#endif // NOCCTRAP
       // Read info passed back up to us by the lower fork, depending on the
       // function requested.  The same number of items must be read from the
       // pipe in the same order as the lower fork put them there.  Trying to
       // read something that's not there makes the program hang
       // uninterruptibly.  Pay close attention -- notice how we fall through
       // some of the cases rather than break; that's deliberate.
  switch (cx) {
#ifdef CK_TRIGGER
      ...
      if (pid) {
        ...
        if (x < 0) {
          printf("ERROR: Failure to kill pid %ld: %s, errno=%d\n", (long)pid,
                 ck_errstr(), errno);
          debug(F111, "CONNECT error killing stale pid", ck_errstr(), errno);
        }
        pid = (PID_T)0;
      }
...
#undef USECCJMPBUF
  ...
  case CEV_MEBIN: // Child sends me_binary change
    read(xpipe[0], (char *)&TELOPT_ME(TELOPT_BINARY),
  ...
  case CEV_UBIN: // Child sends u_binary change
    read(xpipe[0], (char *)&TELOPT_U(TELOPT_BINARY),
  ...
  case CEV_ADL: // Autodownload
    apcactive = APC_LOCAL;
    if (!justone) {
      debug(F100, "CONNECT autodownload at parent", "", 0);
    }
  ...
  case CEV_APC: // Application Program Command
    ...
    obc = 0;
    obp = obuf;
    *obuf = NUL; // Because port fork flushed
    sjval = CEV_APC;
...
static int ckcputf() { // Dump the output buffer
  int x = 0;
...
static void concld() {
  ...
  close(xpipe[0]);
  xpipe[0] = -1; // Child - prevent future reads
#ifdef DEBUG
  if (deblog) {
  ...
  while (1) { // Fresh read, wait for a character.
    ...
    if (network && (nettype == NET_SX25)) {
      ...
      if ((c == IAC) && network && IS_TELNET()) {
        ...
        if ((tx = tn_doop((CHAR)(c & 0xff), duplex, ckcgetc)) == 0) {
          if (me_bin != TELOPT_ME(TELOPT_BINARY)) {
            debug(F101, "CONNECT TELNET me_bin", "", TELOPT_ME(TELOPT_BINARY));
            pipemsg(CEV_MEBIN); // Tell parent
            write(xpipe[1], &TELOPT_ME(TELOPT_BINARY),
                  sizeof(TELOPT_ME(TELOPT_BINARY)));
            ck_sndmsg(); // Tell the parent fork
          } else if (u_bin != TELOPT_U(TELOPT_BINARY)) {
            debug(F101, "CONNECT TELNET u_bin", "", TELOPT_U(TELOPT_BINARY));
            pipemsg(CEV_UBIN); // Tell parent
            write(xpipe[1], &TELOPT_U(TELOPT_BINARY),
                  sizeof(TELOPT_U(TELOPT_BINARY)));
            ck_sndmsg(); // Tell the parent fork
          }
          continue;
...
#ifdef CK_AUTODL
        // Autodownload.  Check for Kermit S packet prior to translation, since
        // that can change the packet and make it unrecognizable (as when the
        // terminal character set is an ISO 646 one)...  Ditto for Zmodem start
        // packet.
        if (autodl // Autodownload enabled?
          ...
          if (k) {
            ...
            if (k == PROTO_K) {
              ...
              debug(F101, "CONNECT autodownload justone", "", justone);
              // Construct the APC command
              sprintf(apcbuf, "set proto %s, %s, set proto %s", ptab[k].p_name,
                      ksign ? "server" : "receive", ptab[protocol].p_name);
              ...
              debug(F101, "CONNECT write xpipe apclength", "", apclength);
              write(xpipe[1], (char *)&apclength, sizeof(apclength));
              debug(F110, "CONNECT write xpipe apcbuf", apcbuf, 0);
              write(xpipe[1], apcbuf, apclength + 1);

              // Copy our input buffer to the parent fork

              debug(F101, "CONNECT autodownload complete ibc", "", ibc);
              debug(F101, "CONNECT autodownload complete obc", "", obc);
      ...
      noserver:
        ...
        // 2. An Esc; we can't write it because it might be the beginning of an
        // APC.
        // 3. The character following an Esc, in which case we write Esc, then
        // char, but only if we have not just entered an APC sequence.
...
int conect() {
  ...
  signal(CK_FORK_SIG,
         SIG_IGN); // Initial CK_FORK_SIG handling,
                   // The following ttimoff() call should not be necessary, but
                   // evidently there are cases where a timer is left active and
                   // then goes off, taking a longjmp to nowhere after the
                   // program's stack has changed.  In any case, this is safe
  ...
  *kbp = NUL; // No need to preserve them between
  ...
  if (msgflg) {
    ...
    if (seslog) {
      ...
      case XYFT_D:
        ...
        break;
      case XYFT_T:
        s = slogts ? "timestamped-text" : "text";
        break;
      default:
        s = "binary";
      }
  ...
  if ((n = ttvt(speed, flow)) < 0) { // Enter "virtual terminal" mode
    if (!network) {
      ...
      if (ttopen(ttname, // Open it again...
                 &local, network ? -nettype : mdmtyp, 0) < 0) {
  ...
  if (pid) { // pid != 0, so I am the upper fork.
             // Before doing anything significant, the child fork must wait for
             // a go-ahead character from xpipe[0].  Before starting to wait, we
             // have enough time to clear buffers and set up the signal handler.
             // When done with this, we will allow the child to continue by
             // satisfying its pending read.
    //
    ...
    // etc to          parent via          xpipe, for preservation until the
    // next entry to          this module, to          ensure that          no
    // characters are          lost between          CONNECT sessions.
    ...
    // simply because there is no other method portable among all UNIX
    // variations. Not threads, not select(), ...  (Yes, select() is more common
    // now; it might actually be worth writing a variation of this module that
    // works like BSD Telnet, one fork, driven by select()).
    ibp = ibuf;                   // Clear ibuf[].
    ...
    close(xpipe[1]);
    xpipe[1] = -1; // Parent - prevent future writes
...
#endif // ANYX25
       // Here is the big loop that gets characters from the keyboard and sends
       // them  out the communication device.  There are two components to the
       // communication  path: the connection from the keyboard to C-Kermit, and
       // from C-Kermit to  the remote computer.  The treatment of the 8th bit
       // of keyboard characters  is governed by SET COMMAND BYTESIZE (cmdmsk).
       // The treatment of the 8th bit  of characters sent to the remote is
       // governed by  SET TERMINAL BYTESIZE  (cmask).   This distinction was
       // introduced in edit  5A(164).
      while (active) {
#ifndef NOSETKEY
...
#ifndef NOSETKEY
        // Note: kmptr is NULL if we got character c from the keyboard, and it
        // is not NULL if it came from a macro.  In the latter case, we must
        // avoid expanding it again.
        if (!kmptr && macrotab[c]) { // Macro definition for c?
          kmptr = macrotab[c];       // Yes, set up macro pointer
          continue;                  // and restart the loop,
        } else {
          c = keymap[c]; // else use single-char keymap
        }
#endif // NOSETKEY
        if (
#ifndef NOSETKEY
       ...
       // Local-to-intermediate
          ...
          for (i = 0; i < outxcount; i++) {
            ...
            if (sosi) {                // Shift-In/Out selected?
              ...
              if (c == SO) {
                outshift = 1; // User typed SO
              }
              if (c == SI) {
                outshift = 0; // User typed SI
              }
            }
...
#ifdef TNCODE
            // If user types the 0xff character (TELNET IAC), it must be
            // doubled.
            else                             // Not CR
...
#define AYT 246
    ...
    case 's': // Status
...
#endif // NETCMD
        ckmakmsg(temp, TMPLEN, " ", (network ? "Host" : "Device"), ": ",
                 ttname);
      conoll(temp);
      if (!network && speed >= 0L) {
      ...
      ckmakmsg(temp, // (would not be safe for sprintf)
               TMPLEN, " Session log: ", *sesfil ? sesfil : "(none)", NULL,
               NULL);
      conoll(temp);
#ifndef NOSHOW
      if (!network) {
