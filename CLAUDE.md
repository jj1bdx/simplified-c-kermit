...
This tree has been heavily **simplified** (2026-06-12 → 2026-06-16) on the premise that it is Linux/macOS/Unix-only: Windows/OS2/VMS, Android, SSL/TLS+Kerberos, K&R C, obsolete platform macros, dead makefile targets, and `#ifdef COMMENT` blocks were removed, and the pre-ANSI macro layer (`_PROTOTYP`, `VOID`/`CKVOID`/`CONST`, `SIGTYP`/`SIGRETURN`) was expanded away. `BASE_20260616.md` is the consolidated overview; per-change reports live in the dated `*_2026*.md` files in the repo root. Linux/macOS/Unix platform code is otherwise unaffected.
