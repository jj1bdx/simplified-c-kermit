...
long pty_getpty(int *fd, char *slave, int slavelength) {
  ...
  if (openpty(fd, &slavefd, slave, (struct termios *)0, (struct winsize *)0)) {
    pty_master_fd = *fd;
    return (1);
  }
  close(slavefd);
    ...
    for (cp = "pqrstuvwxyzPQRST"; *cp; cp++) {
      ...
      for (i = 0; i < 16; i++) {
        ...
        *fd = open(slavebuf, O_RDWR | O_NDELAY);
        if (*fd < 0) {
          debug(F111, "pty_getpty() pty master open error", slavebuf, errno);
          continue;
        }
...
long pty_open_slave(char *slave, int *fd, int fc) {
  int vfd, testfd;
  long retval;
#ifdef CK_POSIX_SIG
  struct sigaction sa;
