...
errprep(offset, buf) unsigned short offset;
char *buf;
{
  ...
  if (lseek(strfile, (long)offset, 0) < 0 || read(strfile, buf, BUFLEN) <= 0)
    goto oops;
}

/* extracted string front end for printf() */
/*VARARGS1*/
strprerror(fmt, va_alist) int fmt;
va_dcl {
  va_list ap;
  char buf[BUFLEN];
...
/*VARARGS1*/
strsrerror(fmt, obuf, va_alist) int fmt;
char *obuf;
va_dcl {
  char buf[BUFLEN];
  va_list ap;
...
/*VARARGS1*/
strfrerror(fmt, fd, va_alist) int fmt;
FILE *fd;
va_dcl {
  va_list ap;
  char buf[BUFLEN];
