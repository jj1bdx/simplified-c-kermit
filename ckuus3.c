...
extern int local, server, success, dest, sleepcan, inserver, flow, autoflow,
    binary, parity, escape, what, turn, duplex, backgrd, hwparity, stopbits,
    turnch, mdmtyp, network, quiet, nettype, carrier, debses, debtim, cdtimo,
    nlangs, bgset, pflag, msgflg, cmdmsk, xsuspend, techo, pacing, xitwarn,
    xitsta, outesc, cmd_cols, cmd_rows, ckxech, xaskmore, haveline, didsetlin,
    isguest, mdmsav, clearrq, saveask, debmsg;
...
static struct keytab logintab[] = {{"password", LOGI_PSW, CM_INV},
...
static struct keytab pfxtab[] = {"all",     PX_ALL, 0, "cautious", PX_CAU, 0,
                                 "minimal", PX_WIL, 0, "none",     PX_NON, 0};
#endif // CK_SPEED
#endif // NOXFER
...
#ifdef CK_SPEED
struct keytab ctltab[] = {"prefixed",   1, 0, // Note, the values are important.
                          "unprefixed", 0, 0};
#endif // CK_SPEED

static struct keytab oldnew[] = {"new", 0, 0, "old", 1, 0};

#define MCH_FIFO 1
#define MCH_DOTF 2
struct keytab matchtab[] = {{"dotfile", MCH_DOTF, 0}, {"fifo", MCH_FIFO, 0}};
int nmatchtab = (sizeof(matchtab) / sizeof(struct keytab));

#ifndef NOSPL
static struct keytab functab[] = {"diagnostics", FUNC_DI, 0,
                                  "error",       FUNC_ER, 0};
...
struct keytab chktab[] = {
    ...
    "blank-free-2",
    4,
    CM_INV, // B = 12-bit checksum, no blanks
    "force-3",
    5,
    CM_INV // F = Force CRC on ALL packets
};
static int nchkt = (sizeof(chktab) / sizeof(struct keytab));

struct keytab rpttab[] = {
    // SET REPEAT
...
struct keytab dbgtab[] = {
    "linelength", DEB_LEN, CM_INV, "m",          DEB_MSG, CM_ABR | CM_INV,
    "message",    DEB_MSG, 0,      "msg",        DEB_MSG, CM_INV,
    "off",        DEB_OFF, 0,      "on",         DEB_ON,  0,
    "session",    DEB_SES, 0,      "timestamps", DEB_TIM, 0};
...
struct keytab spdtab[] = {"0",       0,      CM_INV, "110", 11, 0,
#ifdef BPS_115K
                          "115200",  11520,  0,
#endif // BPS_115K
                          "1200",    120,    0,
...
struct keytab tnspdtab[] = { // RFC 2217 TELNET COMPORT Option
    ...
    "1200",   120,   0, "14400", 1440, 0, "19200", 1920, 0, "230400", 23040, 0,
    "2400",   240,   0, "28800", 2880, 0, "300",   30,   0, "38400",  3840,  0,
    "460800", 46080, 0, "4800",  480,  0, "57600", 5760, 0, "600",    60,    0,
    "9600",   960,   0};
...
struct keytab tnflotab[] = { // SET FLOW-CONTROL keyword table
    ...
    "dtr/cts", FLO_DTRT, 0, "keep",     FLO_KEEP, 0, "none", FLO_NONE, 0,
    "rts/cts", FLO_RTSC, 0, "xon/xoff", FLO_XONX, 0};
...
struct keytab flotab[] = {         // SET FLOW-CONTROL keyword table
    ...
    "dtr/cts",   FLO_DTRT, 0,
#endif // CK_DTRCTS
    "keep",      FLO_KEEP, 0,      "none", FLO_NONE, 0,
#ifdef CK_RTSCTS
    "rts/cts",   FLO_RTSC, 0,
#endif // CK_RTSCTS
    "xon/xoff",  FLO_XONX, 0,      "",     0,        0};
int nflo = (sizeof(flotab) / sizeof(struct keytab)) - 1;

//  Handshake characters

struct keytab hshtab[] = {"bell", 007, 0, "code", 998, 0, "cr", 015, 0, "esc",
                          033, 0, "lf", 012, 0, "none", 999,
                          0, // (can't use negative
                             // numbers)
...
extern char *dialini, *dialmstr, *dialmprmt, *dialdir[], *dialcmd, *dialnpr,
    *dialdcon, *dialdcoff, *dialecon, *dialecoff, *dialhcmd, *dialx3, *dialhwfc,
    *dialswfc, *dialnofc, *dialtone, *dialpulse, *dialname, *diallac;
...
char *dialtfc[MAXTOLLFREE] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
int ntollfree = 0;

char *dialpxx[MAXPBXEXCH] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
int ndialpxx = 0;

char *diallcac[MAXLOCALAC] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                              NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                              NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
                              NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
int nlocalac = 0;

static struct keytab drstrtab[] = {"international", 5, 0, "local", 2, 0,
                                   "long-distance", 4, 0, "none",  6, 0};

static struct keytab dcnvtab[] = {"ask", 2, 0, "off", 0, 0, "on", 1, 0};

struct keytab setmdm[] = {"capabilities",
                          XYDCAP,
                          0,
                          "carrier-watch",
                          XYDMCD,
...
struct keytab voltab[] = {"high", 3, 0, "low", 1, 0, "medium", 2, 0};

struct keytab mdmcap[] = {"at-commands",
                          CKD_AT,
                          0,
                          "compression",
                          CKD_DC,
...
struct keytab dialtab[] = { // SET DIAL table
    "area-code",
    XYDLAC,
    0, // Also still includes items
    "compression",
    XYDDC,
    CM_INV, // that were moved to SET MODEM,
    "confirmation",
    XYDCNF,
    0, // but they are CM_INVisible...
    "connect",
    XYDCON,
    0,
    "convert-directory",
    ...
    "mnp-enable",
    XYDMNP,
    CM_INV, // obsolete but still accepted
#ifdef MDMHUP
    "modem-hangup",
    XYDMHU,
    CM_INV,
...
#ifdef MDMHUP
struct keytab mdmhang[] = {"dtr",          0, 0, "modem-command", 1, 0,
                           "rs232-signal", 0, 0, "v24-signal",    0, CM_INV};
#endif // MDMHUP
...
struct keytab dial_fc[] = {"auto",    FLO_AUTO, 0, "none",     FLO_NONE, 0,
                           "rts/cts", FLO_RTSC, 0, "xon/xoff", FLO_XONX, 0};

struct keytab dial_m[] = { // DIAL METHOD
    "auto",  XYDM_A, 0, "default", XYDM_D, 0,
    "pulse", XYDM_P, 0, "tone",    XYDM_T, 0};
int ndial_m = (sizeof(dial_m) / sizeof(struct keytab));
#endif // NODIAL

#ifdef CK_TAPI
struct keytab tapitab[] = { // Top-Level Microsoft TAPI
    "configure-line", XYTAPI_CFG, 0, "dialing-properties", XYTAPI_DIAL, 0};
...
struct keytab wdottab[] = { // cont'd
    "/match-dot-files", 1, 0, "/no-match-dot-files", 0, 0};
...
#ifdef ANYX25
struct keytab x25tab[] = {"call-user-data",    XYUDAT, 0,
...
struct keytab padx3tab[] = {
    "break-action", PAD_BREAK_ACTION, 0, "break-character", PAD_BREAK_CHARACTER,
    0, "character-delete", PAD_CHAR_DELETE_CHAR, 0, "cr-padding",
    PAD_PADDING_AFTER_CR, 0, "discard-output", PAD_SUPPRESSION_OF_DATA, 0,
    "echo", PAD_ECHO, 0, "editing", PAD_EDITING, 0, "escape", PAD_ESCAPE, 0,
    "forward", PAD_DATA_FORWARD_CHAR, 0, "lf-padding", PAD_PADDING_AFTER_LF, 0,
    "lf-insert", PAD_LF_AFTER_CR, 0, "line-delete", PAD_BUFFER_DELETE_CHAR, 0,
    "line-display", PAD_BUFFER_DISPLAY_CHAR, 0, "line-fold", PAD_LINE_FOLDING,
    0, "pad-flow-control", PAD_FLOW_CONTROL_BY_PAD, 0, "service-signals",
    PAD_SUPPRESSION_OF_SIGNALS, 0, "timeout", PAD_DATA_FORWARD_TIMEOUT, 0,
    // Speed is read-only
    "transmission-rate", PAD_LINE_SPEED, 0, "user-flow-control",
    PAD_FLOW_CONTROL_BY_USER, 0};
int npadx3 = (sizeof(padx3tab) / sizeof(struct keytab));
#endif // IBMX25
#endif // ANYX25

#ifdef TLOG
static struct keytab vbtab[] = {"brief", 0, 0, "ftp", 2, 0, "verbose", 1, 0};
int nvb = (sizeof(vbtab) / sizeof(struct keytab));
#endif // TLOG

#ifdef CKSYSLOG
static struct keytab syslogtab[] = {
    "all",         SYSLG_CX, 0, "commands",    SYSLG_CM, 0,
    "connection",  SYSLG_AC, 0, "debug",       SYSLG_DB, 0,
    "dial",        SYSLG_DI, 0, "file-access", SYSLG_FA, 0,
    "file-create", SYSLG_FC, 0, "login",       SYSLG_LI, 0,
    "none",        SYSLG_NO, 0, "protocol",    SYSLG_PR, 0};
int nsyslog = (sizeof(syslogtab) / sizeof(struct keytab));
#endif // CKSYSLOG

// Parity keyword table

struct keytab partbl[] = {"even",     'e', 0,
#ifdef HWPARITY
                          "hardware", 'H', 0,
#endif // HWPARITY
                          "mark",     'm', 0, "none",  0,   0,
...
struct keytab cdtab[] = {"home", XYCD_H, 0,      "message", XYCD_M,
                         0,      "path", XYCD_P, 0};
int ncdtab = (sizeof(cdtab) / sizeof(struct keytab));

struct keytab cdmsg[] = {"file", 2, 0, "off", 0, 0, "on", 1, 0};
int ncdmsg = (sizeof(cdmsg) / sizeof(struct keytab));

static struct keytab xittab[] = {
    // SET EXIT
    "hangup",        3, 0, // ...HANGUP
    "message",       4, 0, // ...MESSAGE
...
struct keytab rltab[] = {"local", 1, 0, // ECHO values
                         "off",   0, CM_INV, "on", 1, CM_INV, "remote", 0, 0};
int nrlt = (sizeof(rltab) / sizeof(struct keytab));

// Incomplete File Disposition table

struct keytab ifdtab[] = {"discard", SET_OFF, 0, "keep", SET_ON, 0};

struct keytab ifdatab[] = {"auto", SET_AUTO, 0,      "discard", SET_OFF,
                           0,      "keep",   SET_ON, 0};

char *ifdnam[] = {"discard", "keep", "auto"};

// SET TAKE parameters table
static struct keytab taktab[] = {
    "echo", 0, 0,     "error", 1, 0, "off", 2, CM_INV, // For compatibility
...
static struct keytab smactab[] = {"echo", 0, 0, "error", 1, 0};
#endif // NOSPL

#ifndef NOSCRIPT
static struct keytab scrtab[] = {"echo", 0, 0};
#endif // NOSCRIPT
...
static struct keytab srvtab[] = {
    "cd-message",   XYSERC, 0, "display",   XYSERD, 0, "get-path", XYSERP, 0,
    "idle-timeout", XYSERI, 0, "keepalive", XYSERK, 0, "login",    XYSERL, 0,
    "timeout",      XYSERT, 0};
...
struct keytab xfrmtab[] = { // TRANSFER MODE table
    "automatic", XMODE_A, 0, "manual", XMODE_M, 0};

#ifdef LOCUS
extern int locus, autolocus;

static struct keytab locustab[] = {
    {"auto", 2, 0}, {"local", 1, 0}, {"remote", 0, 0}};
...
struct keytab lstab[] = {"forced", 2, 0, "off", 0, 0, "on", 1, 0};
...
static struct keytab tnbugtab[] = { // TELNET BUG table
    "auth-krb5-des",         4, 0, "binary-me-means-u-too", 0, 0,
    "binary-u-means-me-too", 1, 0, "infinite-loop-check",   2, 0,
    "sb-implies-will-do",    3, 0};

#ifdef CK_ENVIRONMENT
static struct keytab tnenvtab[] = { // TELNET ENVIRONMENT table
    "acct",    TN_ENV_ACCT, 0,      "display",    TN_ENV_DISP, 0,
    "job",     TN_ENV_JOB,  0,      "location",   TN_ENV_LOC,  0,
    "off",     TN_ENV_OFF,  CM_INV, "on",         TN_ENV_ON,   CM_INV,
    "printer", TN_ENV_PRNT, 0,      "systemtype", TN_ENV_SYS,  0,
    "user",    TN_ENV_USR,  0,      "uservar",    TN_ENV_UVAR, 0,
    "",        0,           0};
...
#define TN_NL_BIN 3
#define TN_NL_NVT 4
static struct keytab tn_nlmtab[] = {
    // TELNET NEWLINE-MODE table
...
struct keytab tnopttab[] = {"authentication",
                            CK_TN_AU,
                            CM_INV,
                            "binary-mode",
                            CK_TN_BM,
...
struct keytab tnoptsw[] = {"/client", CK_TN_CLIENT, 0,
...
struct keytab desttab[] = { // SET DESTINATION
    ...
    "nowhere",   DEST_N, 0,
#endif // CALIBRATE
    "printer",   DEST_P, 0,      "screen", DEST_S, 0};
...
static int parsdir(int cx) {
  ...
  while (1) {
    if (cx != 2) { // Dialing or Network Directory
      ...
      zzstring("\\v(home)", &s, &y);
#endif // UNIX
      y = cmifip("Names of one or more directory files, separated by spaces",
                 "", &s, &x, 0, dirpath, xxstring);
...
static int setdcd() {
  int x, y, z = 0;
  if ((y = cmkey(crrtab, ncrr, "", "automatic", xxstring)) < 0) {
    return (y);
  }
  if (y == CAR_ON) {
...
int getyesno(char *msg, int flags) {
  ...
  do {
    ...
    switch (flags) {
    case 0:
      y = cmkey(q0yesno, nq0yesno, "", "", NULL);
      break;
    case 1:
      y = cmkey(q1yesno, nq1yesno, "", "", NULL);
      break;
    case 2:
      y = cmkey(q2yesno, nq2yesno, "", "", NULL);
      break;
    default:
      y = cmkey(q3yesno, nq3yesno, "", "", NULL);
    }
    if (y < 0) {
      if (y == -4) { // EOF
        z = y;
...
int uq_file(char *preface, char *fprompt, int fc, char **help, char *dflt,
  ...
  do {
    ...
    if (x < 0) { // Parse error
      ...
      if (x == -4) { // EOF
        ...
        printf(fc == 2 ? " Please enter a directory name.\n"
...
static struct keytab uchmodsw[] = {
    "/directories", CHM_DIR, 0,      "/dotfiles", CHM_DOT, 0,
    "/files",       CHM_FIL, 0,      "/list",     CHM_LIS, 0,
    "/nolist",      CHM_NOL, 0,      "/nopage",   CHM_NOP, 0,
    "/page",        CHM_PAG, 0,      "/quiet",    CHM_QUI, CM_INV,
    "/recursive",   CHM_REC, 0,      "/simulate", CHM_SIM, 0,
    "/type",        CHM_TYP, CM_ARG, "/verbose",  CHM_VRB, CM_INV,
};
...
int douchmod() {
  ...
  cmfdbi(&sw,    // First FDB - command switches
         _CMKEY, // fcode
         "Octal file permission code, or switch", "", // default
         "",                                          // addtl string data
...
struct keytab sexptab[] = {"depth-limit",          1, 0, "echo-result", 0, 0,
...
char *dosexp(char *s) // s = S-Expression
{
...
#endif // FNFLOAT
          s2 = (fpflag && !sexptrunc) ? fpformat(fpresult, 0, 0)
                                      : ckfstoa(result);
        }
...
long dologshow(int fc) // SHOW (current) CONNECTION
{
...
void dolognet() {
  ...
  if (n + m < CXLOGBUFL - 1) { // SAFE
    p = cxlogbuf + n;
    sprintf(p, " %s %s T=%s N=%s H=%s P=%s ", uu, ckgetpid(), s, ttname, myhost,
            port);
  } else {
...
static int dialstr(char **p, char *msg) {
  int x;
  char *s;
...
void initmdm(int x) {
  ...
  if (usermdm && p) { // USER-DEFINED: copy info from specified template
    ...
    if (dialini) {
      makestr(&dialini, NULL); // Init-string
    }
    if (dialmstr) {
      makestr(&dialmstr, NULL); // Dial-mode-str
    }
    if (dialmprmt) {
      makestr(&dialmprmt, NULL); // Dial-mode-pro
    }
    if (dialcmd) {
      makestr(&dialcmd, NULL); // Dial-command
    }
    if (dialdcon) {
      makestr(&dialdcon, NULL); // DC ON command
    }
    if (dialdcoff) {
      makestr(&dialdcoff, NULL); // DC OFF command
    }
    if (dialecon) {
      makestr(&dialecon, NULL); // EC ON command
    }
    if (dialecoff) {
      makestr(&dialecoff, NULL); // EC OFF command
    }
    if (dialhcmd) {
      makestr(&dialhcmd, NULL); // Hangup command
    }
    if (dialhwfc) {
      makestr(&dialhwfc, NULL); // Flow control...
    }
    if (dialswfc) {
      makestr(&dialswfc, NULL); //
    }
    if (dialnofc) {
      makestr(&dialnofc, NULL); //
    }
    if (dialtone) {
      makestr(&dialtone, NULL); // Dialing method
    }
    if (dialpulse) {
      makestr(&dialpulse, NULL); //
    }
    if (dialname) {
      makestr(&dialname, NULL); // Modem name
    }
    if (dialaaon) {
      makestr(&dialaaon, NULL); // Autoanswer On
    }
    if (dialaaoff) {
      makestr(&dialaaoff, NULL); // Autoanswer Off
    }
    if (dialx3) {
      makestr(&dialx3, NULL); // Ignore dialtone
    }
    if (dialspon) {
      makestr(&dialspon, NULL); // Speaker On
    }
    if (dialspoff) {
      makestr(&dialspoff, NULL); // Speaker Off
    }
    if (dialvol1) {
      makestr(&dialvol1, NULL); // Low volume
    }
    if (dialvol2) {
      makestr(&dialvol2, NULL); // Medium volume
    }
    if (dialvol3) {
      makestr(&dialvol3, NULL); // High volume
    }
    if (dialini2) {
      makestr(&dialini2, NULL); // Init string 2
    }
  }
...
int setmodem() { // SET MODEM
  ...
  cmfdbi(&k1, _CMKEY, "Modem parameter", "", "", nsetmdm, 0, xxstring, setmdm,
         &k2);
  ...
  case XYDMHU: // DIAL MODEM-HANGUP
    if ((y = cmkey(mdmhang, 4, "how to hang up modem", "modem-command",
                   xxstring)) < 0) {
      return (y);
    }
    if ((x = cmcfm()) < 0) {
  ...
  case XYDCAP:
    ...
    y = 0;
    while (y != -3) {
      if ((y = cmkey(mdmcap, nmdmcap, "capability of modem", "", xxstring)) <
          0) {
        if (y == -3) {
          break;
        } else {
  ...
  case XYDTYP:
    ...
    if (y == dialudt) { // User-defined modem type
      if ((x = cmkey(mdmtab, nmdm, "based on existing modem type", "unknown",
                     xxstring)) < 0) {
        return (x);
      }
    }
...
setdial(int y) {
 ...
 for example, \"set dial dial-command ATDT%s\\13\"",
    ...
    if (y > 0) {                // If there is any text (left),
      ...
      if (x == y) {
        printf("?Dial-command must contain \"%cs\" for phone number.\n", '%');
        return (-9);
      }
    }
  ...
  case XYDEC:  // DIAL ERROR-CORRECTION
    ...
    if (x > 0) {
      if (!dialec) {
        dialdc = 0; // OFF also turns off compression
      }
    }
    return (x);
  ...
  case XYDDC: // DIAL COMPRESSION
    ...
    if (x > 0) {
      if (dialdc) {
        dialec = 1; // ON also turns on error correction
      }
    }
    return (x);
  ...
  case XYDSTR: // DIAL STRING
    if ((y = cmkey(mdmcmd, nmdmcmd, "", "", xxstring)) < 0) {
      return (y);
    }
    switch (y) {
    ...
    case XYDS_EC: // Error correction ON/OFF
      ...
      sprintf(tmpbuf, "Modem's command to %sable %s", // SAFE
              ...
              (y == XYDS_DC)
                  ? "compression"
                  : ((y == XYDS_EC) ? "error-correction" : "autoanswer"));
      if (x) {
        if (y == XYDS_DC) {
    ...
    case XYDS_NF: //    no flow control
      return (
          dialstr(&dialnofc, "Modem's command to disable local flow control"));
    case XYDS_PX: //    prefix
    ...
    case XYDS_DT: //    tone dialing
      return (
          dialstr(&dialtone, "Command to configure modem for tone dialing"));
    case XYDS_DP: //    pulse dialing
      return (
          dialstr(&dialpulse, "Command to configure modem for pulse dialing"));
    case XYDS_MS: //    dial mode string
      return (dialstr(&dialmstr, "Command to enter dial mode"));
    case XYDS_MP: //    dial mode prompt
      return (dialstr(&dialmprmt, "Modem response upon entering dial mode"));
    ...
    case XYDS_VO: // VOLUME LOW
      ...
      case 0:
      case 1:
        return (dialstr(&dialvol1, "Command for low modem speaker volume"));
      case 2:
        return (dialstr(&dialvol2, "Command for medium modem speaker volume"));

      case 3:
        return (dialstr(&dialvol3, "Command for high modem speaker volume"));
      default:
        return (-2);
      }

    case XYDS_ID: // IGNORE-DIALTONE
      return (dialstr(&dialx3, "Command to tell modem to ignore dialtone"));

    case XYDS_I2: // PREDIAL-INIT
      return (
          dialstr(&dialini2, "Command to send to modem just prior to dialing"));
  ...
  case XYDFC: // DIAL FLOW-CONTROL
    if ((y = cmkey(dial_fc, 4, "", "auto", xxstring)) < 0) {
      return (y);
    }
    if ((x = cmcfm()) < 0) {
      return (x);
  ...
  case XYDRTM:
    y = cmnum("Number of times to try dialing a number", "1", 10, &x, xxstring);
  ...
  case XYDINT:
    y = cmnum("Seconds to wait between redial attempts", "30", 10, &x,
              xxstring);
  ...
  case XYDPXI: { // DIAL PBX-INTERNAL-PREFIX
    int x;
    if ((x = cmtxt("Internal-call prefix of PBX you are calling from", "", &s,
                   NULL)) < 0) { // Don't evaluate
      return (x);
    }
#ifndef NOSPL
    if (*s) {
      ...
      if (*s == '\\') {
        ...
        if (c != 'f' && ckstrcmp(s, "\\v(d$px)", 8, 0) &&
            ckstrcmp(s, "\\v(d$pxx)", 9, 0) && ckstrcmp(s, "\\v(d$p)", 7, 0)) {
          x = TMPBUFSIZ;
          zzstring(s, &p, &x);
          s = tmpbuf;
        }
  ...
  case XYDPXO: // DIAL PBX-OUTSIDE-PREFIX
    return (
        dialstr(&dialpxo, "Outside-line prefix of PBX you are calling from"));
  ...
  case XYDTFC: {          // DIAL TOLL-FREE-AREA-CODE
    ...
    for (n = 0; n < MAXTOLLFREE; n++) {
      if (n == 0) {
        ...
        m = (y == XYDTFC) ? "Another toll-free area code"
  ...
  case XYDTFP: // TOLL-FREE-PREFIX
    return (dialstr(&dialtfp, " Long-distance prefix for toll-free dialing"));

  case XYDCON: // CONNECT
    z = -1;
    if ((y = cmkey(crrtab, ncrr, "", "auto", xxstring)) < 0) {
      return (y);
    }
    if (y != CAR_OFF) { // AUTO or ON?
      if ((z = cmkey(qvtab, nqvt, "", "verbose", xxstring)) < 0) {
        return (z);
      }
    }
    if ((x = cmcfm()) < 0) {
  ...
  case XYDRSTR: // RESTRICT
    if ((y = cmkey(drstrtab, 4, "", "none", xxstring)) < 0) {
      return (y);
    }
    if ((x = cmcfm()) < 0) {
      return (x);
  ...
  case XYDLLAC: {        // Local area-code list
    ...
    for (n = 0; n < MAXLOCALAC; n++) {
      if ((x = cmfld("Area code to which calls from your area are local", "",
                     &s, xxstring)) < 0) {
        break;
      }
      if (s) {
  ...
  case XYDPAC:
    y = cmnum("Milliseconds to pause between each character sent to dialer", "",
              10, &x, xxstring);
    return (setnum(&dialpace, x, y, 9999));

#ifndef NOSPL
  case XYDMAC:
    if ((x = cmfld("Name of macro to execute just prior to dialing", "", &s,
                   xxstring)) < 0) {
      if (x == -3) {
        s = NULL;
      } else {
  ...
  case XYDTOCC: { // Tone country codes
    ...
    for (n = 0; n < MAXTPCC; n++) {
      if (n == 0) {
        m = (y == XYDPUCC) ? "Country code where Pulse dialing is required"
...
shomodem() {
  ...
  if (p) {
    ...
    if (dialfc == FLO_NONE) {
      printf("none\n");
    } else if (dialfc == FLO_XONX) {
      printf("xon/xoff\n");
    } else if (dialfc == FLO_RTSC) {
      printf("rts/cts\n");
    } else if (dialfc == FLO_AUTO) {
      printf("auto\n");
    }
    printf(" Modem hangup-method:    %s\n",
           dialmhu ? "modem-command" : "rs232-signal");
...
dotapi() {
  ...
  case XYTAPI_CFG: { // TAPI CONFIGURE-LINE
    ...
    if (tapilinetab && _tapilinetab && ntapiline > 0) {
      ...
      if ((y = cmkey(_tapilinetab, ntapiline, "TAPI device name", s,
                     xxstring)) < 0) {
        return (y);
      }
      lineID = y;
...
settapi() {
  ...
  if ((y = cmkey(settapitab, nsettapitab, "MS TAPI option", "", xxstring)) <
      0) {
    return (y);
  }
  switch (y) {
  ...
  case XYTAPI_INA:
    y = cmnum("seconds of inactivity before auto-disconnect", "0", 10, &x,
              xxstring);
    return (setnum(&tapiinactivity, x, y, 65535));
  case XYTAPI_BNG:
    y = cmnum("seconds to wait for credit card tone", "8", 10, &x, xxstring);
...
static struct keytab varevaltab[] = {{"recursive", 1, 0}, {"simple", 0, 0}};
...
int setvareval() {
  ...
  if ((x = cmkey(varevaltab, nvarevaltab,
                 "Method for evaluating \\%x and \\&x[] variables", "",
                 xxstring)) < 0) {
    return (x);
  }
...
#ifdef STOPBITS
struct keytab stoptbl[] = {{"1", 1, 0}, {"2", 2, 0}};
#endif // STOPBITS

static struct keytab sertbl[] = {{"7E1", 0, 0},  {"7E2", 1, 0},  {"7M1", 2, 0},
                                 {"7M2", 3, 0},  {"7O1", 4, 0},  {"7O2", 5, 0},
                                 {"7S1", 6, 0},  {"7S2", 7, 0},
#ifdef HWPARITY
                                 {"8E1", 9, 0},  {"8E2", 10, 0},
#endif // HWPARITY
...
static struct keytab extprotab[] = {{"handler", EXP_HANDLER, 0},
...
static int setproto() { // Select a file transfer protocol
  // char * s = NULL;
  int x = 0, y;
  ...
  if (x && y != PROTO_K) {
    printf("?Sorry, REDIRECT capability required for external protocols.\n");
    return (-9);
  }
  if ((x = protofield(
           ptab[y].h_b_init,
  ...
  if (y == PROTO_K) {
    if ((x = protofield(
             ptab[y].h_x_init,
             "Optional command to send to host to start Kermit server", p3)) <
        0) {
      if (x == -3) {
        goto protoexit;
      } else {
  ...
  if (y != PROTO_K) {
    ...
    if ((x = protofield(
             ptab[y].p_t_scmd,
             "External command to SEND in TEXT mode with this protocol", p5)) <
        0) {
      if (x == -3) {
        goto protoexit;
      } else {
...
static struct keytab setexternhandler[] = {
    {"automatic", 0, 0}, {"pty", 1, 0}, {"system", 2, 0}};

int setextern() { // SET EXTERNAL-PROTOCOL
  int x, y;
  ...
  case EXP_TIMO:
    y = cmnum("Inactivity timeout, seconds,", ckitoa(DEF_EXP_TIMO), 10, &x,
              xxstring);
...
int setdest() {
  int x, y;
  if ((y = cmkey(desttab, ndests, "", "disk", xxstring)) < 0) {
    return (y);
  }
  if ((x = cmcfm()) < 0) {
    return (x);
...
#ifdef DECNET
struct keytab dnettab[] = {"cterm", NP_CTERM, 0, "lat", NP_LAT, 0};
...
static int setprinter(int xx) {
  ...
  while (1) {       // Parse 0 or more switches
    ...
    case PRN_PIP:
      ...
      if (n == PRN_FIL) { // File, check accessibility
        ...
        pv[PRN_FLO].ival = FLO_KEEP;
                                ...
                                "DOS printer device name"
                                         : // Help message
                                (n == PRN_PIP ? "Program name" : "Filename"),
                            n == PRN_DOS ? "PRN" : // Default
                                "",
                            &s, xxstring)) < 0) {
        goto xsetprn;
      }
    ...
    case PRN_TRM: // /END-OF-JOB:string
      ...
      if ((x = cmfld("String (enclose in braces if it contains spaces)", "", &s,
                     xxstring)) < 0) {
        goto xsetprn;
      }
      s = brstrip(s);
    ...
    case PRN_FLO:
      ...
      if ((x = cmkey(flotab, nflo, "Serial printer-port flow control",
                     "rts/cts", xxstring)) < 0) {
        goto xsetprn;
      }
      pv[n].ival = x;
    ...
    case PRN_SPD:
      ...
      if ((x = cmkey(spdtab, // Speed (no default)
                     nspd, "Serial printer-port interface speed", "9600",
                     xxstring)) < 0) {
        goto xsetprn;
      }
      pv[n].ival = x;
    ...
    case PRN_PAR:
      ...
      if ((x = cmkey(partbl, npar, "Serial printer-port parity", "none",
                     xxstring)) < 0) {
        goto xsetprn;
      }
      pv[n].ival = x;
...
static struct keytab sshtab[] = { // SET SSH command table
    ...
    {"kerberos5", SSH_K5, 0},        // SSH_FEAT_ADV_KERBEROS5
    {"krb4", SSH_K4, CM_INV},        // SSH_FEAT_ADV_KERBEROS4
    {"krb5", SSH_K5, CM_INV},        // SSH_FEAT_ADV_KERBEROS5
    {"privileged-port", SSH_PRP, 0}, // SSH_FEAT_FROM_PRIV_PRT
    {"proxy-command", SSH_PXC, 0},   // SSH_FEAT_PROXY_CMD
    {"quiet", SSH_SHH, 0},
    {"strict-host-key-check", SSH_SHK, 0},
    {"use-openssh-config", SSH_CFG, 0}, // SSH_FEAT_OPENSSH_CONF
    {"v1", SSH_V1, 0},                  // SSH_FEAT_SSH_V1
    {"v2", SSH_V2, 0},
    {"verbose", SSH_VRB, 0},
...
#define SSH_GSSAPI_DELEGATE 1
#define SSH_GSSAPI_KEYEX 2
...
void shossh() {
  ...
  ssh_ver = ssh_get_iparam(SSH_IPARAM_VER);

#ifndef SSH_DLL
  printf("\nSSH is built in (%s):\n\n", ssh_impl_ver());
#else
  if (!ssh_avail()) {
    printf("\nNo SSH backend loaded!");
    printf("\nUse \"ssh load\" to specify a backend to make SSH commands "
           "available");
  } else {
    printf("\nSSH is loaded (%s):\n\n", ssh_impl_ver());
    printf(" ssh backend:                     %s\n",
           showstring((char *)ssh_dll_name()));
    printf(" ssh backend version:             %s\n",
           showstring((char *)ssh_dll_ver()));
#endif // SSH_DLL
  printf(" ssh host:                        %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_HST)));
  printf(" ssh port:                        %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_PRT)));
  printf(" ssh command:                     %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_CMD)));
  if (ssh_feature_supported(SSH_FEAT_AGENT_FWD)) {
    printf(" ssh agent-forwarding:            %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_AFW)));
  }
  ...
  if (ssh_feature_supported(SSH_FEAT_GSSAPI_DELEGAT)) {
    printf(" ssh gssapi delegate-credentials: %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_GSD)));
  }
  if (ssh_feature_supported(SSH_FEAT_GSSAPI_KEYEX)) {
    printf(" ssh gssapi key-exchange:         %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_GKX)));
  }
  ...
  printf(" ssh heartbeat interval:          %d\n",
         ssh_get_iparam(SSH_IPARAM_HBT));
  if (ssh_feature_supported(SSH_FEAT_ADV_KERBEROS4)) {
    printf(" ssh k4 tgt-passing:              %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_K4TGT)));
  }
  if (ssh_feature_supported(SSH_FEAT_ADV_KERBEROS5)) {
    printf(" ssh k5 tgt-passing:              %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_K5TGT)));
  }
  if (ssh_feature_supported(SSH_FEAT_FROM_PRIV_PRT)) {
    printf(" ssh privileged-port:             %s\n",
           showooa(ssh_get_iparam(SSH_IPARAM_PRP)));
  }
  if (ssh_feature_supported(SSH_FEAT_PROXY_CMD)) {
    printf(" ssh proxy command:               %s\n",
           showstring((char *)ssh_get_sparam(SSH_SPARAM_PXC)));
  }
  printf(" ssh quiet:                       %s\n",
         showoff(ssh_get_iparam(SSH_IPARAM_SHH)));
  printf(" ssh strict-host-key-check:       %d\n",
         ssh_get_iparam(SSH_IPARAM_SHK));
  if (ssh_feature_supported(SSH_FEAT_OPENSSH_CONF)) {
    printf(" ssh use-openssh-config:          %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_CFG)));
  }
  printf(" ssh verbose:                     %d\n",
         ssh_get_iparam(SSH_IPARAM_VRB));
  ...
  if (ssh_feature_supported(SSH_FEAT_SSH_V1)) {
    ...
    printf(" ssh v1 cipher:                   %s\n",
           showstring((char *)ssh_get_sparam(SSH_SPARAM_1_CIF)));
    printf(" ssh v1 global-known-hosts-file:  %s\n",
           showstring((char *)ssh_get_sparam(SSH_SPARAM_1_GNH)));
    printf(" ssh v1 user-known-hosts-file:    %s\n",
           showstring((char *)ssh_get_sparam(SSH_SPARAM_1_UNH)));
  }

  printf("\n");
  printf(" ssh v2 authentication:           %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_AUTH)));
  if (ssh_feature_supported(SSH_FEAT_REKEY_AUTO)) {
    printf(" ssh v2 auto-rekey:               %s\n",
           showoff(ssh_get_iparam(SSH_IPARAM_2_ARK)));
  }
  printf(" ssh v2 ciphers:                  %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_CIF)));
  printf(" ssh v2 command-as-subsystem:     %s\n",
         showoff(ssh_get_iparam(SSH_IPARAM_CAS)));
  printf(" ssh v2 global-known-hosts-file:  %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_GNH)));
  printf(" ssh v2 hostkey-algorithms:       %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_HKA)));
  printf(" ssh v2 key-exchange-methods:     %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_KEX)));
  printf(" ssh v2 mac:                      %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_MAC)));
  printf(" ssh v2 user-known-hosts-file:    %s\n",
         showstring((char *)ssh_get_sparam(SSH_SPARAM_2_UNH)));
#ifdef SSH_DLL
}
#endif // SSH_DLL
#else
...
static int dosetssh() {
...
#ifdef SSH_DLL
  if (!ssh_avail()) {
    printf("\nNo SSH backend DLL loaded. SSH commands unavailable. Use the SSH "
           "LOAD command\n");
  ...
  // backend
  for (z = 0; z < nsshtab; z++) {
    if ((sshtab[z].kwval == SSH_V1 || sshtab[z].kwval == SSH_VER) &&
        !ssh_feature_supported(SSH_FEAT_SSH_V1)) {
      // "set ssh v1" and "set ssh version" commands.
      sshtab[z].flgs = CM_INV;
    } else if ((sshtab[z].kwval == SSH_XFW &&
                !ssh_feature_supported(SSH_FEAT_X11_FWD)) ||
               (sshtab[z].flgs == SSH_XAL &&
                !ssh_feature_supported(SSH_FEAT_X11_XAUTH))) {
      ...
      sshtab[z].flgs = CM_INV;
    } else if (sshtab[z].kwval == SSH_K4 &&
               !ssh_feature_supported(SSH_FEAT_ADV_KERBEROS4)) {
      // "set ssh kerberos4" commands.
      sshtab[z].flgs = CM_INV;
    } else if (sshtab[z].kwval == SSH_K5 &&
               !ssh_feature_supported(SSH_FEAT_ADV_KERBEROS5)) {
      // "set ssh kerberos5" commands.
      sshtab[z].flgs = CM_INV;
    } else if (sshtab[z].kwval == SSH_PXC &&
               !ssh_feature_supported(SSH_FEAT_PROXY_CMD)) {
      // "set ssh command" commands.
      sshtab[z].flgs = CM_INV;
    } else if (sshtab[z].kwval == SSH_CFG &&
               !ssh_feature_supported(SSH_FEAT_OPENSSH_CONF)) {
      // "set ssh use-openssh-config" commands.
      sshtab[z].flgs = CM_INV;
    } else if (sshtab[z].kwval == SSH_PRP &&
               !ssh_feature_supported(SSH_FEAT_FROM_PRIV_PRT)) {
      // "set ssh use-openssh-config" commands.
      sshtab[z].flgs = CM_INV;
  ...
  for (z = 0; z < nsshv2tab; z++) {
    if (sshv2tab[z].kwval == SSH2_ARK &&
        !ssh_feature_supported(SSH_FEAT_REKEY_AUTO)) {
      // "set ssh v2 auto-rekey"
      sshv2tab[z].flgs = CM_INV;
    }
  }

  // Hide any "set ssh gssapi" commands not supported by the currently loaded
  // SSH backend
  ...
  case SSH_CMD: // Command
    if ((x = cmtxt("Command to start ssh", "ssh -e none", &s, xxstring)) < 0) {
      return (x);
    }
    makestr(&sshcmd, s);
    return (success = 1);
  ...
  case SSH_DYF: // Dynamic Forwarding
    if (!ssh_feature_supported(SSH_FEAT_DYN_PORT_FWD)) {
      printf("\r\nDynamic port forwarding is not supported by the current SSH "
             "backend\r\n");
      return (-9);
    }
  ...
  case SSH_GWP: // Gateway ports
    if (!ssh_feature_supported(SSH_FEAT_DYN_PORT_FWD)) {
      printf("\r\nPort forwarding is not supported by the current SSH "
             "backend\r\n");
      return (-9);
    }
    return (success = set_ssh_iparam_on(SSH_IPARAM_GWP));
  ...
  case SSH_GSS: // GSSAPI
    ...
    switch (y) {
    case SSH_GSSAPI_DELEGATE: // Delegate credentials
      if (!ssh_feature_supported(SSH_FEAT_GSSAPI_DELEGAT)) {
        printf("\r\nDelegating GSSAPI credentials is not supported by the "
               "current SSH backend\r\n");
        return (-9);
      }
      return (success = set_ssh_iparam_on(SSH_IPARAM_GSD));
    case SSH_GSSAPI_KEYEX: // key-exchange
      if (!ssh_feature_supported(SSH_FEAT_GSSAPI_KEYEX)) {
        printf("\r\nConfiguration of GSSAPI Key Exchange is not supported by "
               "the current SSH backend\r\n");
        return (-9);
      }
      return (success = set_ssh_iparam_on(SSH_IPARAM_GKX));
    }
    if ((x = cmcfm()) < 0) {
      return (x);
    }
  ...
  case SSH_K5: // Kerberos V
    if (cx == SSH_K4 && !ssh_feature_supported(SSH_FEAT_X11_FWD)) {
      printf("\r\nNo Kerberos 4 settings are supported by the current SSH "
             "backend\r\n");
      return (-9);
    } else if (cx == SSH_K5 && !ssh_feature_supported(SSH_FEAT_X11_FWD)) {
      printf("\r\nNo Kerberos 5 settings are supported by the current SSH "
             "backend\r\n");
      return (-9);
    }
    ...
    switch (y) {
    case 1:
      return (success = (cx == SSH_K4) ? set_ssh_iparam_on(SSH_IPARAM_K4TGT)
                                       : set_ssh_iparam_on(SSH_IPARAM_K5TGT));
    }
    if ((x = cmcfm()) < 0) {
      return (x);
    }
  ...
  case SSH_PRP: // Privd port
    if (!ssh_feature_supported(SSH_FEAT_FROM_PRIV_PRT)) {
      printf("\r\nConnecting from a privileged port not supported by the "
             "current SSH backend\r\n");
      return (-9);
    }
    return (success = set_ssh_iparam_on(SSH_IPARAM_PRP));

  case SSH_SHH: // Quiet
    return (success = set_ssh_iparam_on(SSH_IPARAM_SHH));
  ...
  case SSH_SHK: // Strict Host Key Check
    ...
    success = ssh_set_iparam(SSH_IPARAM_SHK, y);
    return (success);
  ...
  case SSH_HBT:
    ...
    success = ssh_set_iparam(SSH_IPARAM_HBT, z);
    return (success);
  ...
  case SSH_V1: // SSH V1
    ...
    switch (y) {
    case SSH1_K54:
      return (success = set_ssh_iparam_on(SSH_IPARAM_K5_IS_K4));
    case SSH1_CIF: { // Not a list
      struct keytab *ssh1ciphers;
      int nssh1ciphers;
      ktab_ret ktr;

      ktr = ssh_get_keytab(SSH_KTAB_V1_CIPHERS);
      if (ktr.rc != 0) {
        printf("\r\nFailed to get options from the SSH backend\r\n");
        return (-9);
      }
      ssh1ciphers = ktr.ktab;
      nssh1ciphers = ktr.ktab_len;
      ...
      if (y == -3) {
        ssh_set_sparam(SSH_SPARAM_1_CIF, NULL);
      } else {
        ...
        ssh_set_sparam(SSH_SPARAM_1_CIF, ssh1ciphers[x].kwd);
      }
      return (1);
    }
    case SSH1_GNH:
    case SSH1_UNH:
      ...
      switch (y) {
      case SSH1_GNH:
        ssh_set_sparam(SSH_SPARAM_1_GNH, s);
        break;
      case SSH1_UNH:
        ssh_set_sparam(SSH_SPARAM_1_UNH, s);
        break;
      }
      return (1);
    }
  ...
  case SSH_V2: // SSH V2
    ...
    case SSH2_ARK:
      if (!ssh_feature_supported(SSH_FEAT_REKEY_AUTO)) {
        printf("\r\nConfiguring automatic re-key is not supported by the "
               "current SSH backend\r\n");
        return (-9);
      }
      return (success = set_ssh_iparam_on(SSH_IPARAM_2_ARK));
    case SSH2_AUT: { // Authentication
#define TMPCNT 12
      int i, j, tmp[TMPCNT];
      struct keytab *ssh2aut;
      int nssh2aut;
      ktab_ret ktr;

      ktr = ssh_get_keytab(SSH_KTAB_V2_AUT);
      if (ktr.rc != 0) {
        printf("\r\nFailed to get options from the SSH backend\r\n");
        return (-9);
      }
      ssh2aut = ktr.ktab;
      nssh2aut = ktr.ktab_len;

      for (i = 0; i < TMPCNT; i++) {
        tmp[i] = 0;
      }
      for (i = 0; i < TMPCNT; i++) {
        if ((y = cmkey(ssh2aut, nssh2aut, "Authentication method", "",
                       xxstring)) < 0) {
          if (y == -3) {
            break;
          }
      ...
      if (i > 0) {
        ...
        ssh_set_sparam(SSH_SPARAM_2_AUTH, ssh2_auth);
        free(ssh2_auth);
      }
      return (success = 1);
#undef TMPCNT
...
#define TMPCNT 12
      int i, j, tmp[TMPCNT];
      struct keytab *ssh2ciphers;
      int nssh2ciphers;
      ktab_ret ktr;

      ktr = ssh_get_keytab(SSH_KTAB_V2_CIPHERS);
      if (ktr.rc != 0) {
        printf("\r\nFailed to get options from the SSH backend\r\n");
        return (-9);
      }
      ssh2ciphers = ktr.ktab;
      nssh2ciphers = ktr.ktab_len;
      ...
      for (i = 0; i < TMPCNT; i++) {
        if ((y = cmkey(ssh2ciphers, nssh2ciphers, "", "", xxstring)) < 0) {
          if (y == -3) {
            break;
          }
          return (y);
      ...
      if (i > 0) {
        ...
        ssh_set_sparam(SSH_SPARAM_2_CIF, ssh2_cif);
        free(ssh2_cif);
      }
      return (success = 1);
#undef TMPCNT
...
#define TMPCNT 12
      int i, j, tmp[TMPCNT];
      struct keytab *ssh2macs;
      int nssh2macs;
      ktab_ret ktr;

      ktr = ssh_get_keytab(SSH_KTAB_V2_MACS);
      if (ktr.rc != 0) {
        printf("\r\nFailed to get options from the SSH backend\r\n");
        return (-9);
      }
      ssh2macs = ktr.ktab;
      nssh2macs = ktr.ktab_len;
      ...
      if (i > 0) {
        ...
        ssh_set_sparam(SSH_SPARAM_2_MAC, ssh2_mac);
        free(ssh2_mac);
      }
      return (success = 1);
#undef TMPCNT
...
#define TMPCNT 12
      int i, j, tmp[TMPCNT];
      struct keytab *hkatab;
      int nhkatab;
      ktab_ret ktr;

      ktr = ssh_get_keytab(SSH_KTAB_HKA);
      if (ktr.rc != 0) {
        printf("\r\nFailed to get options from the SSH backend\r\n");
        return (-9);
      }
      hkatab = ktr.ktab;
      nhkatab = ktr.ktab_len;
      ...
      for (i = 0; i < TMPCNT; i++) {
        if ((y = cmkey(hkatab, nhkatab, "", "", xxstring)) < 0) {
          if (y == -3) {
            break;
          }
          return (y);
      ...
      if (i > 0) {
        ...
        ssh_set_sparam(SSH_SPARAM_2_HKA, ssh2_hka);
        free(ssh2_hka);
      }
      return (success = 1);
#undef TMPCNT
...
#define TMPCNT 24
      int i, j, tmp[TMPCNT];
      struct keytab *sshkextab;
      int nsshkextab;
      ktab_ret ktr;

      ktr = ssh_get_keytab(SSH_KTAB_KEX);
      if (ktr.rc != 0) {
        printf("\r\nFailed to get options from the SSH backend\r\n");
        return (-9);
      }
      sshkextab = ktr.ktab;
      nsshkextab = ktr.ktab_len;
      ...
      for (i = 0; i < TMPCNT; i++) {
        if ((y = cmkey(sshkextab, nsshkextab, "", "", xxstring)) < 0) {
          if (y == -3) {
            break;
          }
          return (y);
      ...
      if (i > 0) {
        ...
        ssh_set_sparam(SSH_SPARAM_2_KEX, ssh2_kex);
        free(ssh2_kex);
      }
      return (success = 1);
#undef TMPCNT
    ...
    case SSH2_UNH:
      ...
      switch (y) {
      case SSH2_GNH:
        ssh_set_sparam(SSH_SPARAM_2_GNH, s);
        break;
      case SSH2_UNH:
        ssh_set_sparam(SSH_SPARAM_2_UNH, s);
        break;
      default:
        return (success = 0);
      }
  ...
  case SSH_VER: // Version
    ...
    ssh_set_iparam(SSH_IPARAM_VER, y); // 0 == AUTO
#ifndef SSHTEST
    sl_ssh_ver_saved = 0;
#endif // SSHTEST
    return (success = 1);
  ...
  case SSH_XFW: // X11-forwarding
    ...
    success = set_ssh_iparam_on(SSH_IPARAM_XFW);
#ifndef SSHTEST
    if (success) {
      sl_ssh_xfw_saved = 0;
    }
  ...
  case SSH_XAL: // SSH Xauth Location
    ...
    ssh_set_sparam(SSH_SPARAM_XAL, s);
    return (success = 1);

  case SSH_PXC: // SSH Proxy Command
    if (!ssh_feature_supported(SSH_FEAT_PROXY_CMD)) {
      printf("\r\nSSH proxy command feature not supported by the current SSH "
             "backend\r\n");
      return (-9);
    }
  ...
  case SSH_CFG: // Use OpenSSH Config
    if (!ssh_feature_supported(SSH_FEAT_OPENSSH_CONF)) {
      printf("\r\nOpenSSH configuration not supported by the current SSH "
             "backend\r\n");
      return (-9);
    }
    return (success = set_ssh_iparam_on(SSH_IPARAM_CFG));
#endif // SSHBUILTIN
...
int doprm(int xx, int rmsflg) {
  ...
  case XYNET: { // SET NETWORK
...
#ifdef SSH_DLL
    if (z == NET_SSH && !ssh_avail()) {
      printf("\n?Sorry, no SSH backend DLL loaded; SSH commands are disabled. "
             "Use the SSH LOAD\n"
    ...
    if (z == NET_PIPE || // Named pipe -- also get pipename
      ...
      if (z == NET_BIOS) {
        ...
        if (y - NETBIOS_NAME_LEN > 0) {
          ...
          rc = NCBAddName(NetbeuiAPI, &ncb, NetBiosAdapter, NetBiosName);
          if (rc) {
            printf(
    ...
    if (
        ...
        (nettype != NET_SLAT) &&
#endif // SUPERLAT
        (nettype != NET_SX25) && (nettype != NET_VX25) &&
#ifdef IBMX25
        (nettype != NET_IX25) &&
#endif // IBMX25
#ifdef SSHBUILTIN
  ...
  case XYTCP: {
    ...
    case XYTCP_HTTP_PROXY: {
      ...
      cmfdbi(&sw,    // First FDB - switches
             _CMKEY, // fcode
             "HTTP proxy server host[:port] or switch", "", // default
             "",                                            // addtl string data
      ...
      cmfdbi(&tx, _CMTXT,                         // fcode
             "HTTP proxy server host[:port]", "", // default
             "",                                  // addtl string data
             0,                                   // addtl numeric data 1
             0,                                   // addtl numeric data 2
             xxstring, NULL, NULL);
      while (1) {
        ...
        case UPW_AGENT:
          ...
          ckstrncpy((n == UPW_USER) ? ubuf : ((n == UPW_PASS) ? pbuf : abuf), s,
    ...
    case XYTCP_ADDRESS:
      if ((y = cmtxt("preferred IP Address for TCP connections", "", &s,
                     xxstring)) < 0) {
        return (y);
      }
#ifdef IKSDCONF
    ...
    case XYTCP_DONTROUTE:
      if ((z = cmkey(onoff, 2, "", "off", xxstring)) < 0) {
        return (z);
      }
      if ((y = cmcfm()) < 0) {
        return (y);
    ...
    case XYTCP_LINGER:
      ...
      if (z) { // if on, we need a timeout value
        if ((x = cmnum("Linger timeout in 10th of a millisecond", "0", 10, &y,
                       xxstring)) < 0) {
          return (x);
        }
      } else {
  ...
  case XYBUF: { // BUFFERS
    ...
    if (sb < 0) {
      if (*atmbuf == '-') {
        ...
        printf("?Integer overflow, use a smaller number please\n");
      }
      return (-9);
    } else if (sb < 80) {
      printf("?Too small\n");
  ...
  case XYCHKT: // BLOCK-CHECK
    if ((x = cmkey(chktab, nchkt, "", "3", xxstring)) < 0) {
      return (x);
    }
    if ((y = cmcfm()) < 0) {
      return (y);
  ...
  case XYTELOP: {
    ...
    case TELOPT_IBM_SAK:
    case TELOPT_LOGOUT:
      if ((x = cmkey(tnnegtab, ntnnegtab, "desired server state",
                     TELOPT_MODE(tnserver ? TELOPT_DEF_S_ME_MODE(opt)
    ...
    case TELOPT_LFLOW:
    case TELOPT_COMPORT:
      if ((x = cmkey(tnnegtab, ntnnegtab, "desired client state",
                     TELOPT_MODE(!tnserver ? TELOPT_DEF_S_U_MODE(opt)
    ...
    default:
      if ((x = cmkey(tnnegtab, ntnnegtab,
                     tnserver ? "desired server state" : "desired client state",
                     TELOPT_MODE(tnserver ? TELOPT_DEF_S_ME_MODE(opt)
                                          : TELOPT_DEF_C_ME_MODE(opt)),
                     xxstring)) < 0) {
        return (x);
      }
      if ((y = cmkey(tnnegtab, ntnnegtab,
  ...
  case XYTEL: // TELNET
    ...
    switch (z) {
    case CK_TN_EC: // ECHO
      if ((x = cmkey(rltab, nrlt, "initial TELNET echoing state", "local",
                     xxstring)) < 0) {
        return (x);
      }
      if ((y = cmcfm()) < 0) {
    ...
    case CK_TN_TT: // TERMINAL TYPE
      if ((y = cmtxt("terminal type for TELNET connections", "", &s,
                     xxstring)) < 0) {
        return (y);
      }
      if (tn_term) {
    ...
    case CK_TN_ENV: {
      ...
      extern char tn_env_acct[], tn_env_disp[], tn_env_job[], tn_env_prnt[],
          tn_env_sys[];
      extern char *tn_loc;
      ...
      if (x == TN_ENV_UVAR) { // User variables
        ...
        if ((x = cmfld("Name of Environment Variable", "", &s, xxstring)) < 0) {
          return (x);
        }
        makestr(&uvar, s);
        ...
        if ((x = cmtxt("Value of Environment Variable", env, &s, xxstring)) <
            0) {
          return (x);
        }
        if (*s) {
...
#ifdef CK_RECALL
    case SCMD_RCL:
      if ((y = cmnum("maximum number of commands in recall buffer", "10", 10,
                     &x, xxstring)) < 0) {
        return (y);
      }
      if ((y = cmcfm()) < 0) {
  ...
  case XYDEBU: // SET DEBUG { on, off, session }
    ...
    case DEB_MSG: // Debug messages 2010/03/12
      if ((y = cmkey(ooetab, nooetab, "", "on", xxstring)) < 0) {
        return (y);
      }
      if ((x = cmcfm()) < 0) {
        return (x);
  ...
  case XYDUPL: // SET DUPLEX
    if ((y = cmkey(dpxtab, 2, "", "full", xxstring)) < 0) {
      return (y);
    }
    if ((x = cmcfm()) < 0) {
      return (x);
 ...
 or connection type";
  ...
  case XYHAND: // HANDSHAKE
    if ((y = cmkey(hshtab, nhsh, "", "none", xxstring)) < 0) {
      return (y);
    }
    if (y == 998) {
  ...
  case XYMACR: // SET MACRO
    if ((y = cmkey(smactab, 2, "", "", xxstring)) < 0) {
      return (y);
    }
    switch (y) {
    case 0:
      return (success = seton(&mecho));
    case 1:
      return (success = seton(&merror[cmdlvl]));
    default:
      return (-2);
    }
#endif // NOSPL
  ...
  case XYPROM: // SET PROMPT
               // Note: xxstring not invoked here.  Instead, it is invoked every
               // time the prompt is issued.  This allows the prompt string to
               // contain variables that can change, like \v(dir), \v(time),
               // etc.
    ckmakmsg(line, // Default might have trailing space
             LINBUFSIZ, "{", inserver ? ikprompt : ckprompt, "}", NULL);
  ...
  case XYSERV: // SET SERVER items
    if ((y = cmkey(srvtab, nsrvt, "", "", xxstring)) < 0) {
      return (y);
    }
    switch (y) {
    case XYSERI:
      if ((y = cmnum("Number of seconds, or 0 for no idle timeout", "0", 10, &x,
                     xxstring)) < 0) {
        return (y);
      }
      if (x < 0) {
      ...
      return (success = 1);
    case XYSERT:
      if ((y = cmnum("Interval for server NAKs, 0 = none", ckitoa(DSRVTIM), 10,
                     &x, xxstring)) < 0) {
        return (y);
      }
      if (x < 0) {
    ...
    case XYSERC: // SERVER CD-MESSAGE
      x = rmsflg ? cmkey(onoff, 2, "", "", xxstring)
  ...
  case XYTAKE: // SET TAKE
    if ((y = cmkey(taktab, 4, "", "", xxstring)) < 0) {
      return (y);
    }
    switch (y) {
    case 0:
      return (success = seton(&techo));
#ifndef NOSPL
    case 1:
      return (success = seton(&takerr[cmdlvl]));
#else
    case 1:
      return (success = seton(&takerr[tlevel]));
#endif // NOSPL
    case 2:
      techo = 0;
      return (success = 1); // For compatibility with
    case 3:
      techo = 1;
      return (success = 1); // MS-DOS Kermit
    default:
      return (-2);
    }
  ...
  case XYSCRI: // SET SCRIPT
    if ((y = cmkey(scrtab, 1, "", "echo", xxstring)) < 0) {
      return (y);
    }
    switch (y) {
    case 0:
      return (success = seton(&secho));
    default:
      return (-2);
    }
#endif // NOSCRIPT
  ...
  case XYSPEE: // SET SPEED
    lp = line;
    if (local && !network) {
      ckmakmsg(lp, LINBUFSIZ, "Transmission rate for ", ttname, " (bps)", NULL);
    } else {
      ckstrncpy(lp, "Serial-port speed (bps)", LINBUFSIZ);
    }
    zz = -1L;
...
// than 230400.  The argument to ttsspd() should have been a long.
  ...
  case XYXFER:                              // SET TRANSFER
    if ((y = cmkey(rmsflg ? rtstab : tstab, // (or REMOTE SET TRANSFER)
                   rmsflg ? nrts : nts, "", "character-set", xxstring)) < 0) {
      return (y);
    }
    switch (y) {
#ifdef XFRCAN
    case XYX_CAN: // CANCELLATION
      ...
      if (z == 0) { // OFF
        ...
        if ((y = cmnum("How many required to cause cancellation", "2", 10, &z,
                       xxstring)) < 0) {
          return (y);
        }
        if (z < 2) {
  ...
  case XYUNCS: // UNKNOWN-CHARACTER-SET
    if ((y = cmkey(ifdtab, 2, "", "discard", xxstring)) < 0) {
      return (y);
    }
    if ((x = cmcfm()) < 0) {
      return (x);
  ...
  case XYWILD: // WILDCARD-EXPANSION
    if ((y = cmkey(wildtab, nwild, "Wildcard expansion option", "on",
                   xxstring)) < 0) {
      return (y);
    }
    if ((z = cmkey(wdottab, 2,
                   "whether to match filenames that start with \".\"",
                   "/no-match-dot-files", xxstring)) < 0) {
      return (z);
    }
    if ((x = cmcfm()) < 0) {
      return (x);
 ...
 or the word \"all\", or carriage return to complete the list",
          ...
          if (x == -2) {
            ...
            if (!ckstrcmp(atmbuf, "all", 3, 0) || // "ALL"
                !ckstrcmp(atmbuf, "al", 2, 0) || !ckstrcmp(atmbuf, "a", 1, 0)) {
        ...
        if (z == 0) { // If they are saying it is safe...
          ...
          if (((flow == FLO_XONX) &&
            ...
            return (-9);
                   ...
                   (y == CK_CR || (unsigned)y == (unsigned)255)) {
            if (msgflg) {
      ...
      // Get here only if they have made no mistakes.  Copy temporary table back
      // to permanent one, then free temporary table and return successfully.
      for (i = 0; i < 256; i++) {
        ctlp[i] = p[i];
      }
      if (p) {
      ...
      if (z > 0) {
        clearrq = 0; // 199 (see SET PREFIXING)
      }
      return (success = 1);
    default:
      return (-2);
...
#ifndef NOXFER
  case XYREPT:
    if ((y = cmkey(rpttab, 2, "repeat-count compression parameter", "",
                   xxstring)) < 0) {
      return (y);
    }
    switch (y) {
  ...
  case XYALRM: {
    ...
    y = cmnum("Seconds from now, or time of day as hh:mm:ss", "0", 10, &x,
              xxstring);
    x_ifnum = yy;
    if (y < 0) {
    ...
    if (zz > -1L) { // Time of day given?
      x = zz;
      if (zz != (long)x) {
        printf("Sorry, arithmetic overflow - hh:mm:ss not usable on this "
               "platform.\n");
        return (-9);
      }
    }
  ...
  case XYPROTO:
    ...
    // C-Kermit unprefixes control characters automatically on network
    // connections if CLEAR-CHANNEL is ON, which it is by default.  But not all
    // network connections are transparent to all control characters.  For
    // example, the DEC-20, even when you TELNET to it, is sensitive to Ctrl-O
    // and Ctrl-T. If you tell C-Kermit to SET CONTROL PREFIX 15 and/or 20, it
    // doesn't help because CLEAR-CHANNEL is still in effect.  If the user goes
    // to the trouble to set up some prefixing, then Kermit should do what the
    // user said.  In C-Kermit 7.1 Alpha.03 we change the code to set clearrq to
    // 0 if the user gives a SET PREFIXING or SET CONTROL PREFIX command.
...
#ifndef NOSPL
  case XYLOGIN:
    if ((z = cmkey(logintab, 3, "value for login script", "userid", xxstring)) <
        0) {
      return (z);
    }
    x = cmdgquo();
  ...
  case XYBROWSE: // SET BROWSER
  {
...
#endif // CK_APC
    if (p) {
      x = cmifip(xx == XYBROWSE ? "Name of preferred browser"
                                : "Name of preferred ftp client",
                 xx == XYFTP ? "ftp" : "", &s, &y, 0, p, xxstring);
    } else {
      x = cmifi(xx == XYBROWSE ? "Full path of preferred browser"
    ...
    ckstrncpy(line, s, LINBUFSIZ);
    if ((x = cmtxt(xx == XYBROWSE ? "browser command-line options"
  ...
  case XYTLOG: { // SET TRANSACTION-LOG
    ...
    if (x == 0) {
      if ((y = cmtxt("field separator", ",", &s, NULL)) < 0) {
        return (y);
      }
      s = brstrip(s);
      if (*s) {
  ...
  case XYFUNC:
    ...
    switch (x) {
    case FUNC_DI:
      return (seton(&fndiags));
    case FUNC_ER:
      return (seton(&fnerror));
    default:
      return (-2);
    }
#endif // NOSPL
  ...
  case XYSTOP: // STOP-BITS
    if ((x = cmkey(stoptbl, 2, "Stop bits for serial device", "", xxstring)) <
        0) {
      return (x);
    }
    if ((y = cmcfm()) < 0) {
  ...
  case XYSERIAL: {
    ...
    if ((x = cmkey(sertbl, nsertbl,
                   "Serial device character size, parity, and stop bits", "8N1",
                   xxstring)) < 0) {
      return (x);
    }
  ...
  case XYSEXP: {
    ...
    case 1: {
      int i, xx;
      xx = sexpmaxdep;
      if ((y = cmnum("Maximum recursion depth", "1000", 10, &x, xxstring)) <
          0) {
        return (y);
      }
  ...
  case XYLOCUS:
    ...
    if (x == 2 || x == 3) { // AUTO or ASK
      ...
      // setlocus(1,0);
      // we're not changing the locus here
...
int hupok(int x) // Returns 1 if OK, 0 if not OK
{
  int y, z = 1;
  extern int exithangup;
#ifdef UNIX
  ...
  if ((local && xitwarn) ||   // Is a connection open?
    ...
    if (network) {
      ...
      if (needwarn) {
        if (strcmp(ttname, "*")) {
          ckmakmsg(warning, 256, " A network connection to ", ttname,
      ...
      if (needwarn) {
        ckmakmsg(warning, 256, " A serial connection might still be active on ",
                 ttname, ".\n", NULL);
      }
    }
    ...
    if (needwarn || (!x && xitwarn == 2
      ...
      z = uq_ok(warning, x ? "OK to close? " : "OK to exit? ", 3, NULL, 0);
...
void shoctl() { // SHOW CONTROL-PREFIXING
  ...
  for (i = 0; i < 16; i++) {
    ...
    printf("  %3d: %d   %3d: %d ", i + 128, ctlp[i + 128], i + 144,
           ctlp[i + 144]);
    if (i == 15) {
      printf("  255: %d", ctlp[255]);
    }
    printf("\n");
  }
  printf("\n");
