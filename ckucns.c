...
extern int local, escape, duplex, parity, flow, seslog, sessft, debses, mdmtyp,
    ttnproto, cmask, cmdmsk, network, nettype, sosi, tnlm, xitsta, what, ttyfd,
    ttpipe, quiet, backgrd, pflag, tt_crd, tt_lfd, tn_nlm, ttfdflg, tt_escape,
    justone, carrier, ttpty, hwparity;
...
static int active = 0, quitnow = 0, // <esc-char>Q was typed
...
static int escseq = 0;    // 1 = Recognizer is active
                          // static
int inesc[2] = {0, 0};    // State of sequence recognizer
                          // static
int oldesc[2] = {-1, -1}; // Previous state of recognizer
...
static int ckcputf() { // Dump the console output buffer
  int x = 0;
...
#endif // UNICODE
          if (sxo) {
            c = (*sxo)((char)c); // Local-intermediate
          }
          if (rxo) {
            c = (*rxo)((char)c); // Intermediate-remote
          }
          outxbuf[0] = c;
          outxcount = 1;
          outxbuf[outxcount] = NUL;
        ...
        if (autodl // Autodownload enabled?
          ...
          if (k) {
            ...
            if (k == PROTO_K) {
              ...
              goto conret1;
#else
              // Here's another way that doesn't require APC, but then we'll
              // have to change all the other CONNECT modules, and then the
              // mainline code that calls them.
              {
                extern char sstate;
                sstate = ksign ? 'x' : 'v';
                proto();
...
// doesc
{
  ...
  while (1) {
    ...
    case '\03':
      ...
      conoll("");
      conoll("");
      conoll("  WARNING: This version of C-Kermit has no command processor to "
             "escape");
      conoll("  back to.  To return to your local system, log out from the "
             "remote and/or");
      conoll("  use the escape character followed by the letter U to close "
             "(hang Up) the");
      conoll("  connection.  Resuming your session...");
      conoll("");
      return;
#else
      active = 0;
